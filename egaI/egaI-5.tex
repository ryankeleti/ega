...
\[
  \xymatrix{
    \Spec(\kres(y)) \ar[r]^{(g_1,g_2)_S} \ar[d]
...
\begin{enumerate}[label=\emph{(\roman*)}]
    ...
    \item For a morphism $f$ to be separated, it is necessary and sufficient for $f_\mathrm{red}$ \sref{1.5.1.5} to be separated.
\end{enumerate}
\end{prop}
...
Finally, to prove (vi), recall that the preschemes $X_\mathrm{red}\times_{Y_\mathrm{red}}X_\mathrm{red}$ and $X_\mathrm{red}\times_Y X_\mathrm{red}$ are canonically identified \sref{1.5.1.7};
if we denote by $j$ the injection $X_\mathrm{red}\to X$, then the diagram
\[
  \xymatrix{
...
Indeed, if $z$ is a section of $\OO_Y$ over $U$, there exist two integers $m,n\geqslant0$ such that $s^mz$ and $t^nz$ are the restrictions of polynomials in $s$ and $t$ to $U$ \sref{1.1.4.1}, which is clearly possible only if the section $z$ extends to a section over the whole of $Y$, identified with a polynomial in $s$ and $t$.
...
\begin{enumerate}[label=\emph{(\roman*)}]
    ...
    \item \emph{The composition of two morphisms that both have property~\textbf{P} also has property~\textbf{P}.}
...
\oldpage[I]{140}
    ...
    \item \emph{If a morphism $f:X\to Y$ has property~\textbf{P}, then so too does $f_\mathrm{red}$ \sref{1.5.1.5}.}
\end{enumerate}
...
The hypothesis that $f$ has property~\textbf{P} implies, by (ii), that $X_\mathrm{red}\xrightarrow{f_\mathrm{red}}Y_\mathrm{red}\to Y$ has property~\textbf{P};
finally, since a closed immersion is separated \sref{1.5.5.1}[i], $f_\mathrm{red}$ has property~\textbf{P}, by (v).
