...
\begin{enumerate}[label=\emph{(\roman*)}]
  ...
  \item If the hypotheses of Proposition \sref{1.9.5.1} are satisfied and $X$ is irreducible (\emph{resp.} \unsure{integral}), then so is $Y'$.
\end{enumerate}
\end{prop}
...
As $X$ is reduced, $g$ factors as $X\xrightarrow{h} {Y'}_\mathrm{red}\xrightarrow{j'}Y'$, where $j'$ is the canonical injection \sref{1.5.2.2}, and it then follows from the definition of $Y'$ that $Y'_\mathrm{red}=Y'$.
...
When there exists a smaller closed subprescheme $Y$ of $X$ \unsure{containing} a subprescheme $Y$ of $X$, we say that $Y'$ is the \emph{closure} of $Y$ in $X$, when it doesn't lead to any confusion.
...
For all quasi-compact $\OO_X$-algebras $\sh{B}$ of finite type, there exists a quasi-coherent $\OO_X$-submodule $\sh{E}$ of $\sh{B}$ of finite type such that $\sh{E}$ generates \sref[0]{0.4.1.4} the $\OO_X$-algebra $\sh{B}$.
\end{prop}
