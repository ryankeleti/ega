...
\begin{defn}[7.1.2]
\label{0.7.1.2}
In a linearly topologized ring $A$, we say that an ideal $\fk{J}$ is an {\em ideal of
definition} if $\fk{J}$ is open and if, for each neighborhood $V$ of $0$, there exists
a integer $n>0$ such
\oldpage[0\textsubscript{I}]{61}
that $\fk{J}^n\subset V$ {\em (which we express, by abuse of language, by saying that
the sequence $(\fk{J}^n)$ {\em tends to $0$})}. We say that a linearly topologized
...
\hyperref[0.7.1.10]{(7.1.10)} implies that if $f$ is invertible in $A/\fk{J}$,
...
$\fk{J}_0$ and by applying \hyperref[0.7.1.4]{(7.1.4)}, (i). The converse follows
...
\hyperref[0.7.2.12]{(7.2.12)}, $\fk{J}/\fk{J}^2$ is then an $A$-module of
...
from Example \hyperref[0.7.2.12]{(7.2.12)} and from the hypothesis on $M/\fk{m}M$.
...
\hyperref[0.1.7.2]{(1.7.2)}) to the case where $M/\fk{m}M$ is {\em simple}, thus
...
follows from \hyperref[0.7.4.4]{(7.4.4)} applied to the ring $B/\fk{b}$.
\end{proof}
...
$\varprojlim A/\fk{J}_\lambda$ \hyperref[0.7.2.1]{(7.2.1)}. For each
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Set $A_i=A/\fk{J}^{i+1}$, and for $i\leqslant j$, let $u_{ij}$ be the
    ...
    of Proposition \hyperref[0.7.2.7]{(7.2.7)}; as $\fk{J}'$ is the kernel of
...
$B_n=B/\fk{J}^{n+1}B$ \hyperref[0.7.2.11]{(7.2.11)}, and $\fk{J}B/\fk{J}^2 B$
...
$S^{-1}A/S^{-1}\fk{J}_\lambda$, which is necessarily \hyperref[0.1.2.6]{(1.2.6)} of the
form $S^{-1}\fk{a}$, where $\fk{a}\supset\fk{J}_\lambda$. We conclude that
$\fk{a}'=\fk{a}\{S^{-1}\}$. In particular \hyperref[0.1.2.6]{(1.2.6)}:
\end{env}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Set $A_i=A/\fk{J}^{i+1}$, and for $i\leqslant j$, let $u_{ij}$ be the canonical
    ...
    $S_j^{-1}(\fk{J}^{i+1}/\fk{J}^{j+1})$ \hyperref[0.1.3.2]{(1.3.2)}, equal to
...
$S_f^{-1}(\fk{p}/\fk{J})$ \hyperref[0.7.6.9]{(7.6.9)}, which means that
...
$S_g^{-1}A/S_g^{-1}\fk{J}$ is invertible, so (\hyperref[0.7.6.9]{(7.6.9)} and
...
$\fk{H}=\Im(\fk{K}\otimes_A C)+\Im(B\otimes_A\fk{L})$ is an ideal of
definition.
\end{prop}

\begin{proof}
\label{proof-0.7.7.7}
It suffices to show that $\fk{H}^n$ tends to $0$ for the tensor product topology, which follows
...
{\em of finite type}, equipped with the $\fk{J}$-pre-adic topology. For every topological
...
equipped with the $\fk{J}$-pre-adic topology; we know \hyperref[0.7.3.6]{(7.3.6)} that they are
...
topology of $\Hom_A(L,N)$ is the $\fk{J}$-adic \hyperref[0.7.3.2]{(7.3.2)}, we have
...
If we only suppose that $A$ is a {\em Noetherian $\fk{J}$-pre-adic} ring, $M$ and $N$ two
