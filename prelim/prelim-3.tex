...
(E) If $(A,(\rho_\alpha))$ is a solution of a universal mapping problem {\em in
    ...
    also means that the canonical functor $\C\to\Set$ {\em commutes with
...
(F$_0$) {\em For any covering $(U_\alpha)$ of $U\in\fk{B}$ by sets
        ...
        {\em solution to the universal problem} defined in \sref{0.3.1.1} by
...
first a second basis $\fk{B}'$ of the topology of $X$, {\em contained in}
$\fk{B}$, and show that if $\sh{F}''$ denotes the presheaf induced by the
subfamily $(\sh{F}(V))_{V\in\fk{B}'}$, $\sh{F}''$ is {\em canonically
...
$\fk{B}'$ is still a basis of the topology of $X$, so $\sh{F}'(U)$
...
$u_V:\sh{F}(V)\to\sh{G}(V)$ {\em for only the $V\in\fk{B}$} completely
...
that $\fk{H}_\lambda$ is a {\em subsheaf} of $\sh{F}_\lambda$ for each
$\lambda$, $\varprojlim_\lambda\fk{H}_\lambda$ canonically identifies with
...
\begin{env}[3.3.3]
\label{0.3.3.3}
With the notations of \sref{0.3.3.1}, let $V$ be an open set of $X$; it is
...
\begin{env}[3.4.5]
\label{0.3.4.5}
Under the hypotheses of \sref{0.3.4.4}, suppose in addition that $\psi$ is a
...
{\em isomorphisms} for each non-empty open $U\subset X$; we note that $\sh{F}$
...
\begin{env}[3.6.2]
\label{0.3.6.2}
Suppose that $X$ is {\em irreducible} \sref{0.2.1.1}; then the following
...
Indeed, let $\sh{F}$ be a constant presheaf on $X$; if $U$, $V$ are two non-empty
open sets in $X$, then $U\cap V$ is non-empty, so
...
Now let $(U_\alpha)$ be an open cover of $X$ by non-empty open sets and let
...
\begin{env}[3.7.2]
\label{0.3.7.2}
Keeping the notations and hypotheses of \sref{0.3.5.3}, suppose that $\sh{G}$
...
topological spaces {\em on $\fk{B}$} satisfy the condition (F$_0$) of
