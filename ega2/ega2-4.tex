...
\begin{proof}
  ...
  But let $U$ be an open neighbourhood of $x\in X$, and let $\sh{I}$ be a quasi-coherent sheaf of ideals of $\sh{O}_X$ defining a closed subprescheme of $X$ that has $X\setminus U$ as its underlying space \sref[I]{I.5.2.1}.
...
\begin{enumerate}
  ...
  \item[\rm{(d')}] For every quasi-coherent $\sh{O}_X$-module $\sh{F}$ of finite type, there exist integers $n>0$ and $k>0$ such that $\sh{F}$ is isomorphic to a quotient of the $\sh{O}_X$-module $\sh{L}^{\otimes(-n)}\otimes\h{O}_X^k$.
