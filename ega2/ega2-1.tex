...
On the other hand, it follows from \sref{2.1.5.2} that $\Spec(q^*(\sh{B}))$ canonically identifies with $X\times_S S'$; the morphism $v$ is the composition $X'\xrightarrow{w}X\times_S S'\xrightarrow{p_1}X$ of $w$ with the first projection, and the commutativity of \sref{eq:2.1.5.6.1} follows from the definitions.
...
\oldpage[II]{14}
Indeed, the canonical isomorphism \sref{eq:2.1.5.2.1} gives a canonical isomorphism from $p_1^*(\widetilde{\sh{M}})$ to the sheaf on $\Spec(q^*(\sh{B}))$ associated to the $q^*(\sh{B})$-module $q^*(\sh{M})$, and it then suffices to apply \sref{2.1.4.7}.
\end{env}
