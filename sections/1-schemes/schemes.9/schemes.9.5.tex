...
case if $f$ is quasicompact {and separated, or}\marginpar{?} $X$ is locally Noetherian
\emph{(9.2.2)}). Then there exists a smaller subprescheme $Y'$ of $Y$ such that
the canonical injection $j\colon Y'\to Y$ \marginpar{?} $f$ (\emph{or, equivalently
...
$\sheaf{J}'$ of $\O_Y$, such that the injection $j'\colon X''\to Y$ \marginpar{?}
...
of $Y$ such that the canonical injection $j\colon Y'\to Y$ \marginpar{?} $f$, we
