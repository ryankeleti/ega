...
$i_A^S(\mathfrak{a})$, which identifies with $S^{-1}\mathfrak{a}$ \eref{1.3.2}. The map
...
$(S^{-1}A)_{S^{-1}\mathfrak{p}}$ are canonically isomorphic \eref{1.5.1}.
\end{env}
...
\begin{env}{1.2.8}
\label{env-0.1.2.8}
If $A$ is a \emph{reduced} ring \eref{1.1.1}, so is $S^{-1}A$: indeed, if $(x/s)^n=0$ for
...
$S^{-1}A\otimes_A M$ and $S^{-1}A\otimes_A N$ \eref{1.2.5}, $S^{-1}u$ is considered as
...
(\eref{1.2.5} and \eref{1.3.1}), it follows from the permutability of the tensor product and
...
homomorphisms $\bigotimes^n\sh{M}\to\sh{C}$ (for each $n\geq 0$). This is also the sheaf
...
$x$ in $X$} if the stalk $\sh{A}_x$ is a \emph{reduced} ring \eref{1.1.1}; we say that
