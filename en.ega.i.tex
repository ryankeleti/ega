...
Chapter~I, \textsection8. One such extension has been developed by M.~Nagata in
...
\begin{env}{1.0.3}
\label{env-0.1.0.3}
Under the hypothesis of \sref{env}{1.0.2}, if $\mathfrak{J}$ is a left (resp. right) ideal of $A$, we
...
(\sref{env}{1.2.5} and \sref{env}{1.3.1}), it follows from the permutability of the tensor product and
...
$i_M^T=\rho_M^{T,S}\circ i_M^S$; in canonical identification \sref{env}{1.2.5}, $\rho_M^{T,S}$ identifies with
...
\begin{env}{1.4.2}
\label{env-0.1.4.2}
With the same notation, for two $A$-modules $M$, $N$, the diagrams (cf. \sref{env}{1.3.4} and
\sref{env}{1.3.5})
\[
  \xymatrix{
...
$\mathfrak{a}_i$ is the annihilator of $x_i$, it follows from \sref{env}{1.7.3} that
...
then have ${\theta''}^\#=\theta^\#\circ\psi^*({\theta'}^\#)$ \sref{env}{3.5.5}; therefore
...
is an isomorphism for all $x\in X$ \sref{env}{3.7.2}. We verify immediately, thanks to the
...
is a local immersion implies that $\theta_x^\#$ is surjective \sref{env}{4.2.2}, so
...
\oldpage{173}
              ...
              \sref{env}{1.4.1} that there exists a whole number $n>0$ such that, for all
