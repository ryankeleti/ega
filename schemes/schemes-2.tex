...
affine scheme \hyperref[1.1.7.1]{(1.7.1)}.
\end{env}
...
open neighborhood of $x$ contained inside $V\cap W$ \hyperref[1.1.1.10]{(1.1.10)}, (i).
The ringed space $(D(f),\OO_X|D(f))$ is thus an affine scheme, isomorphic to $A_f$
\hyperref[1.1.3.6]{(1.3.6)}, whence the proposition.
\end{proof}
...
are in the same affine open, this is a result of \hyperref[1.1.1.8]{(1.1.8)}.
\end{proof}
...
dense in $Y$, as well as irreducible (\hyperref[0.2.1.1]{(\textbf{0},~2.1.1)} and \hyperref[0.2.1.4]{(\textbf{0},~2.1.4)}); thus by
Corollary \hyperref[1.1.1.14]{(1.1.14)}, $U\cap Y$ is the closure in $U$ of a point $x$, and then
$Y\subset\overline{U}$ is the closure of $x$ in $X$. The uniqueness of the
generic point of $X$ is a result of Proposition \hyperref[1.2.1.4]{(2.1.4)} and of \hyperref[0.2.1.3]{(\textbf{0},~2.1.3)}.
\end{proof}
...
\begin{proof}
\label{proof-1.2.1.7}
This follows directly from Definition \hyperref[1.2.1.2]{(2.1.2)} and Proposition \hyperref[1.2.1.3]{(2.1.3)}.
\end{proof}
...
{\em irreducible and reduced} (cf. \hyperref[1.5.1.4]{(5.1.4)}). We say that a prescheme
...
${\theta''}^\sharp=\theta^\sharp\circ\psi^*({\theta'}^\sharp)$ \hyperref[0.3.5.5]{(\textbf{0},~3.5.5)}. From this
...
injection \hyperref[0.4.1.2]{(\textbf{0},~4.1.2)} of the induced prescheme $(U,\OO_X|U)$ into
...
quickly follows from \hyperref[0.4.1.1]{(\textbf{0},~4.1.1)}.
\end{exm}
...
prescheme $(V_\alpha,\OO_X|V_\alpha)$ to $(S,\OO_S)$, according to Theorem \hyperref[1.1.7.3]{(1.7.3)}.
...
$V_\alpha\cap V_\beta$ \hyperref[1.2.1.3]{(2.1.3)}; it is clear that by composing
...
V_\alpha$ and all $\alpha$ \hyperref[1.1.6.1]{(1.6.1)}, the restriction to $W$ of the morphisms
...
\[
  ...
  \tag{2.2.4.1}
\]
with the notation of \hyperref[0.5.5.2]{(\textbf{0},~5.5.2)} relative to the locally free sheaf
$\OO_X$. In fact, it suffices to verify this formula when $X$ itself is affine,
and then this is nothing but (1.2.2.2).

\begin{prop}[2.2.5]
\label{1.2.2.5}
Under the hypotheses of Proposition \hyperref[1.2.2.4]{(2.2.4)}, let
...
\oldpage[I]{100}
correspondence between $f$-morphisms $\sh{F}\to\sh{G}$ \hyperref[0.4.4.1]{(\textbf{0},~4.4.1)} and
$A$-homomorphisms $M\to(\Gamma(X,\sh{G}))_{[\vphi]}$.
\end{prop}

\begin{proof}
\label{proof-1.2.2.5}
Reasoning as in Proposition \hyperref[1.2.2.4]{(2.2.4)}, we reduce to the case where $X$ is
affine, and the proposition then follows from Proposition \hyperref[1.1.6.3]{(1.6.3)}
and from Corollary \hyperref[1.1.3.8]{(1.3.8)}.
\end{proof}
...
point of $X_i$ (resp. $Y_i$) \hyperref[1.2.1.5]{(2.1.5)}. We say that a morphism
\[
  f=(\psi,\theta):(X,\OO_X)\to(Y,\OO_Y)
\]
is {\em birational} if, for all $i$, $\psi^{-1}(\eta_i)=\{\xi_i\}$ and
$\theta_{\xi_i}^\sharp:\OO_{\eta_i}\to\OO_{\xi_i}$ is an {\em isomorphism}. It
is clear that a birational morphism is dominant \hyperref[0.2.1.8]{(\textbf{0},~2.1.8)}, and
...
\label{1.2.3.1}
\oldpage[I]{101}
It follows from definition \hyperref[1.2.1.2]{(2.1.2)} that every ringed space obtained by
{\em gluing} preschemes \hyperref[0.4.1.6]{(\textbf{0},~4.1.6)} is again a prescheme. In particular, although
...
preschemes $U_{21}\to U_{12}$ corresponding \hyperref[1.2.2.4]{(2.2.4)} to the isomorphism from $B$ to
...
we have that $a\in\overline{\{b\}}$ \hyperref[1.1.1.7]{(1.1.7)}.
\end{env}
...
$\OO_y$ is canonically identified with $B_y$ \hyperref[1.1.3.4]{(1.3.4)}, and the canonical
homomorphism $B\to B_y$ thus corresponds \hyperref[1.1.6.1]{(1.6.1)} to a morphism of preschemes
...
third affine open $W$ containing $y$ and such that $W\subset V\cap V'$ \hyperref[1.2.1.3]{(2.1.3)};
...
\]
is commutative \hyperref[0.1.5.1]{(\textbf{0},~1.5.1)}. The morphism
\[
...
\hyperref[0.2.1.2]{(\textbf{0},~2.1.2)}}; furthermore, if $z=\psi(\mathfrak{p})$, then
...
affine scheme; but then this proposition follows from \hyperref[1.1.6.2]{(1.6.2)}.

{\em We see \hyperref[1.2.1.5]{(2.1.5)} that there is a bijective correspondence between
...
$u=({}^a\vphi,\tilde{\vphi})$, where $\vphi\in\Hom(B,A)$ \hyperref[1.1.7.3]{(1.7.3)}. Further,
...
fractions \hyperref[0.1.2.4]{(\textbf{0},~1.2.4)}. Conversely, to each local homomorphism
$\OO_y\to A$ there exists a unique corresponding morphism
$(\psi,\theta):X\to\Spec(\OO_y)$ such that $\psi(a)=y$ \hyperref[1.1.7.3]{(1.7.3)}, and,
...
so Proposition \hyperref[1.2.4.4]{(2.4.4)} says that:
...
\begin{proof}
\label{proof-1.2.4.7}
We have already seen this when $\mathfrak{a}_y=0$ \hyperref[1.2.4.2]{(2.4.2)}, and it suffices
to apply Corollary \hyperref[1.1.7.5]{(1.7.5)}.
\end{proof}
...
whole of $X$, every {\em invertible} $\OO_X$-module \hyperref[0.5.4.1]{(\textbf{0},~5.4.1)} is
...
It follows from \hyperref[1.2.2.4]{(2.2.4)} that the data of a prescheme over a ring $A$ is
...
say that $X$ {\em dominates} $S$ if $\vphi$ is a dominant morphism \hyperref[1.2.2.6]{(2.2.6)}.
