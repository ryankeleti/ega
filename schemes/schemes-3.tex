...
It follows also from the definitions that every \emph{monomorphism of preschemes} (T,~1.1) is radical.

\begin{env}[3.5.5]
\label{1.3.5.5}
For a morphism $f:X\to Y$ to be radical, it suffices that the condition of Definition \sref{1.3.5.4} hold for every \emph{algebraically closed} field.
...
For a morphism $f=(\psi,\theta):X\to Y$ to be radical, it is necessary and sufficient that, for every morphism $Y'\to Y$, the morphism $f_{(Y')}:X_{(Y')}\to Y'$ be injective (which justifies the terminology of a \emph{universally injective} morphism).
