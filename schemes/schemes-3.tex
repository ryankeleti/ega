...
$Z=\Spec(B\otimes_A C)$, $p_1$, $p_2$ the $S$-morphisms corresponding \hyperref[1.2.2.4]{(2.2.4)} to
...
\begin{proof}
\label{proof-1.3.2.2}
According to \hyperref[1.2.2.4]{(2.2.4)}, it suffices to check that if, to each $A$-homomorphism
...
conclusion of the proposition follows immediately, taking into account \hyperref[1.3.2.1]{(3.2.1)}.
\end{proof}
...
\oldpage[I]{106}
It suffices to apply \hyperref[1.3.2.4]{(3.2.4)} to the canonical injection $S'\to S$.
...
hypotheses and Definition \hyperref[1.3.2.1]{(3.2.1)}.
...
which would finish the proof of Lemma \hyperref[1.3.2.6.2]{(3.2.6.2)}. The images of
...
\]
it follows from Lemma \hyperref[1.3.2.6.1]{(3.2.6.1)} that $W_{\alpha\lambda}\cap W_{\beta\mu}$ and the
...
Applying Lemma \hyperref[1.3.2.6.1]{(3.2.6.1)} to the open sets $U_\alpha\cap U_\beta$ and
...
$W_{ki}\cap W_{kj}$, as it follows from applying Lemma \hyperref[1.3.2.6.1]{(3.2.6.1)} to
...
$f_{ij}=g_i\circ g_j^{-1}$ \hyperref[1.2.3.1]{(2.3.1)}; in addition, we have $g_i(Z_i\cap Z_j)=W_{ij}$.
...
\]
according to Lemma \hyperref[1.3.2.6.1]{(3.2.6.1)}, the restrictions of $p_j$ and $q_j$ to
...
Lemma \hyperref[1.3.2.6.2]{(3.2.6.2)} that $(Z,p,q)$ is a product of $X$ and $Y$.
...
According to Lemma \hyperref[1.3.2.6.3]{(3.2.6.3)}, everything comes down to proving that the products
$X_i\times_S Y_i$ exists for any $i$ and $j$. Set
$X_{ij}=X_i\cap X_j=\vphi^{-1}(S_i\cap S_j)$, $Y_{ij}=Y_i\cap Y_j=\psi^{-1}(S_i\cap S_j)$;
according to Lemma \hyperref[1.3.2.6.1]{(3.2.6.1)}, the product $Z_{ij}=X_{ij}\times_S Y_{ij}$ exists. We now
...
\begin{env}[3.2.6.5]
\label{1.3.2.6.5}
We can now complete the proof of Theorem \hyperref[1.3.2.6]{(3.2.6)}. If $S$ is an {\em affine
...
\hyperref[1.3.2.2]{(3.2.2)}, there similarly exists $X\times_S Y$ by Lemma \hyperref[1.3.2.6.3]{(3.2.6.3)}. If $S$
...
according to the
\oldpage[I]{108}
above; but then the products $X_i\times_S Y_i$ also exist \hyperref[1.3.2.5]{(3.2.5)}, therefore
$X\times_S Y$ similarly exists by Lemma \hyperref[1.3.2.6.4]{(3.2.6.4)}.
\end{env}
\end{proof}
...
\begin{proof}
\label{proof-1.3.2.7}
This follows from Corollary \hyperref[1.3.2.5]{(3.2.5)} and Lemma \hyperref[1.3.2.6.1]{(3.2.6.1)}.
\end{proof}
...
Lemma \hyperref[1.3.2.6.3]{(3.2.6.3)}.
\end{env}

\begin{env}[3.2.9]
\label{1.3.2.9}
\footnote{[Trans] \hyperref[1.3.2.9]{(3.2.9)} is from the errata of EGA~II, on page 221.}
\oldpage[II]{221}
It follows from \hyperref[1.1.8.1]{(1.8.1)} that we can state \hyperref[1.3.2.2]{(3.2.2)} in the following
...
over $S$} (with a definition of $S$-morphisms modeled on that of \hyperref[1.2.5.2]{(2.5.2)}). The
proof of \hyperref[1.3.2.6]{(3.2.6)} also proves that for any two $S$-preschemes $X$, $Y$, the
...
\hyperref[1.3.3.13]{(3.3.13)} and \hyperref[1.3.3.15]{(3.3.15)}, are true without modification in any
...
from \hyperref[1.3.2.6]{(3.2.6)} by induction on $n$, and noting that
...
\begin{env}[3.3.7]
\label{1.3.3.7}
With the notation of \hyperref[1.3.3.6]{(3.3.6)}, for each $S$-morphism $f:X\to Y$, we
...
\[
  ...
  \tag{3.3.9.1}
\]
the functorial nature of the isomorphism defined in \hyperref[1.3.3.9]{(3.3.9)} can
...
Corollary \hyperref[1.3.3.10]{(3.3.10)} can be expressed by the formula
\[
...
products}; it also commutes with sums \hyperref[1.3.2.8]{(3.2.8)}.
...
$S_{(\psi\circ f)}'$; taking into account \hyperref[1.3.3.9]{(3.3.9)} and
\hyperref[1.3.3.4]{(3.3.4)}, we deduce the corollary.
\end{proof}
...
\begin{env}[3.3.14]
\label{1.3.3.14}
With the notation of \hyperref[1.3.3.6]{(3.3.6)}, for each {\em $S$-morphism}
...
it follows in particular from \hyperref[1.3.3.14]{(3.3.14)} that the {\em $X$-sections} of
...
scheme of ring $A_\alpha[T]$ \hyperref[1.3.2.2]{(3.2.2)}, and $u_\alpha$ canonically
corresponds to an $A_\alpha$-homomorphism $A_\alpha[T]\to A_\alpha$
\hyperref[1.1.7.3]{(1.7.3)}. Now, as such a homomorphism is completely determined by the
...
denote it by $P\times_R Q$. Definition \hyperref[1.3.2.1]{(3.2.1)} of the product
of $S$-preschemes can be interpreted, with the notation of \hyperref[1.3.4.1]{(3.4.1)},
via the formula
\[
...
homomorphisms $\OO_x\to A$ for $x\in X$ \hyperref[1.2.2.4]{(2.2.4)}; we say that the
...
$K'$-rational points of $X'=X_{(K')}$ \hyperref[1.3.3.14]{(3.3.14)}.
\end{env}
...
\begin{proof}
\label{proof-1.3.4.6}
In fact, there exist $\kres(s)$-monomorphisms $\kres(x_i)\to K$, all in the same extension $K$ of $\kres(s)$ (Bourbaki, {\em Alg.}, chap.~V, \textsection4, prop.~2).
...
\begin{proof}
\label{proof-1.3.4.7}
The condition is evidently necessary; Lemma \hyperref[1.3.4.6]{(3.4.6)} proves that it is sufficient.
\end{proof}

In other words, if we denote by $(X)$ the underlying set of $X$, we see that we have a canonical {\em surjective} function $(X\times_S Y)\to(X)\times_{(S)}(Y)$; we must point out that this function {\em is not injective} in general; said otherwise, {\em there can exist multiple distinct points $z$ in $X\times_S Y$ having the same projections $x\in X$, $y\in Y$}; we have already seen this when $S$, $X$, and $Y$ are prime spectrums of fields $k$, $K$, and $K'$, since the tensor product $K\otimes_k K'$ has, in general, multiple distinct prime ideals \hyperref[1.3.4.9]{cf. (3.4.9)}.

\begin{cor}[3.4.8]
\label{1.3.4.8}
Let $f:X\to Y$ be an $S$-morphism, $f_{(S')}:X_{(S')}\to Y_{(S')}$ the $S'$-morphism inferred from $f$ by an extension $S'\to S$ of the base prescheme.
Let $p$ ({\em resp. $q$}) be the projection $X_{(S')}\to X$ ({\em resp. $Y_{(S')}\to Y$}); for every subset $M$ of $X$, we have
\[
  q^{-1}(f(M))=f_{(S')}(p^{-1}(M)).
\]
\end{cor}
...
In
\oldpage[I]{114}
fact \hyperref[1.3.3.11]{(3.3.11)}, $X_{(S')}$ is identified with the product $X\times_Y Y_{(S')}$ thanks to the commutative diagram
\[
  \begin{tikzcd}
    X\ar[d,swap,"f"] &
  ...
  \end{tikzcd}
\]
By \hyperref[1.3.4.7]{(3.4.7)}, the relation $q(y')=p(x)$ for $x\in M$ and $y'\in Y_{(S)}$ is equivalent to the existence of some $x'\in X_{(S')}$ such that $p(x')=x$ and $f_{(S')}(x')=y'$, whence the corollary.
\end{proof}

Lemma \hyperref[1.3.4.6]{(3.4.6)} can be made clearer in the following manner:
\begin{prop}[3.4.9]
\label{1.3.4.9}
Let $X$ and $Y$ be two $S$-preschemes, and $x$ a point of $X$ and $y$ a point of $Y$ both above the same point $s\in S$.
The set of points of $X\times_S Y$ with projections $x$ and $y$ is in bijective correspondence with the set of \unsure{types of extensions composed of $\kres(x)$ and $\kres(y)$ considered as extensions of $\kres(s)$} (Bourbaki, {\em Alg.}, chap.~VIII, §8, prop.~2).
\end{prop}

\begin{proof}
\label{proof-1.3.4.9}
Let $p$ ({\em resp. $q$}) be the projection of $X\times_S Y$ into $X$ ({\em resp. $Y$}), and $E$ the subspace $p^{-1}(x)\cap q^{-1}(y)$ of the underlying space of $X\times_S Y$.
First note that the morphisms $\Spec(\kres(x))\to S$ and $\Spec(\kres(y))\to S$ factorise as $\Spec(\kres(x))\to\Spec(\kres(s))\to S$ and $\Spec(\kres(y))\to\Spec(\kres(s))\to S$y since $\Spec(\kres(s))\to S$ is a monomorphism \hyperref[1.2.4.7]{(2.4.7)}, it follows from \hyperref[1.3.2.4]{(3.2.4)} that we have
\[
...
If $i\colon\Spec(\kres(x))\to X$ and $j\colon\Spec(\kres(y))\to Y$ are the canonical morphisms \hyperref[1.2.4.5]{(2.4.5)}, we take for $\alpha$ the map of underlying spaces corresponding to the morphism $i\times_S j$.
...
The verification of the fact that $\alpha\circ\beta$ and $\beta\circ\alpha$ are the identity maps follows from \hyperref[1.2.4.5]{(2.4.5)} and the definition of the product \hyperref[1.3.2.1]{(3.2.1)}.
Finally, we know that $P_0$ is in bijective correspondence with the set of \unsure{types of extensions composed of $\kres(x)$ and $\kres(y)$} (Bourbaki, {\em Alg.}, chap.~VIII, §8, prop.~1).
\end{proof}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item If $f:X\to Y$ is an $S$-morphisms having property $P$, then every $S'$-morphism $f_{(S')}:X_{(S')}\to Y_{(S')}$, inferred from $f$ by an extension of base preschemes, also has property $P$.
\end{enumerate}

Since $f_{(S')}=f\times_S 1_{S'}$, we see that if, for every prescheme $X$, the {\em identity} $1_X$ has property $P$, then (i) implies (ii); since, on the other hand, $f\times_S g$ is the composed morphism
\[
  X\times_S Y \xrightarrow{f\times1_Y} X'\times_S Y \xrightarrow{1_{X'}\times g} X'\times_S Y'
\]
we see that if the {\em composition} of two morphisms has property $P$, then so too does the product $f\times_S g$, and so (ii) implies (i).
\end{env}
...
\begin{proof}
\label{proof-1.3.5.2}
The composition of two surjections being a surjection, it suffices to prove (ii); but this proposition follows quickly from \hyperref[1.3.4.8]{(3.4.8)} applied to $M=X$.
\end{proof}
...
The condition is sufficient because, for all $y\in Y$, it suffices to apply it to a morphism $\Spec(K)\to Y$ corresponding to a monomorphism $\kres(y)\to K$, $K$ being an extension of $\kres(y)$ \hyperref[1.2.4.6]{(2.4.6)}.
Conversely, suppose that $f$ is surjective, and let $y\in Y$ be the image of the unique point of $\Spec(K)$; there exists some $x\in X$ such that $f(x)=y$; we will consider the corresponding monomorphism $\kres(y)\to\kres(x)$ \hyperref[1.2.2.1]{(2.2.1)}; it then suffices to take $K'$ to be the extension of $\kres(y)$ such that there exist $\kres(y)$-monomorphisms from $\kres(x)$ and $K$ to $K'$ (Bourbaki, {\em Alg.}, chap.~V, §4, prop.~2); the morphism $\Spec(K')\to X$ corresponding to $\kres(x)\to K'$ is exactly that for which we search.
\end{proof}

With the language introduced in \hyperref[1.3.4.5]{(3.4.5)}, we can say that {\em every geometric point of $Y$ with values in $K$ comes from a geometric point of $X$ with values in an extension of $K$}.

\begin{defn}[3.5.4]
\label{1.3.5.4}
We say that a morphism $f:X\to Y$ of preschemes is universally injective, or a \unsure{radical} morphism, if, for every field $K$, the corresponding map $X(K)\to Y(K)$ is injective.
\end{defn}

It follows also from the definitions that every {\em monomorphism of preschemes} (T,~1.1) is \unsure{radical}.

\begin{env}[3.5.5]
\label{1.3.5.5}
For a morphism $f:X\to Y$ to be \unsure{radical}, it suffices that the condition of def.~\hyperref[1.3.5.4]{(3.5.4)} hold for every {\em algebraically closed} field.
...
\begin{proof}
\label{proof-1.3.5.6}
Taking into account def.~\hyperref[1.3.5.4]{(3.5.4)}, the proposition reduces to the corresponding assertions for the maps $X(K)\to Y(K)\to Z(K)$, which are evident.
\end{proof}

\begin{prop}[3.5.7]
\label{1.3.5.7}
\begin{enumerate}[label={\rm(\roman*)}]
  \item If the $S$-morphisms $f:X\to X'$ and $g:X\to X'$ are \unsure{radical}, then so too is $f\times_S g$.
  \item If the $S$-morphism $f:X\to Y$ is \unsure{radical}, then so too is $f_{(S')}:X_{(S')}\to Y_{(S')}$ for every extension $S'\to S$ of the base prescheme.
\end{enumerate}
\end{prop}

\begin{proof}
\label{proof-1.3.5.7}
Given \hyperref[1.3.5.1]{(3.5.1)}, it suffices to prove (i).
We have seen \hyperref[1.3.4.2]{(3.4.2.1)} that
\begin{gather*}
...
\begin{prop}[3.5.8]
\label{1.3.5.8}
For a morphism $f=(\psi,\theta):X\to Y$ to be \unsure{radical}, it is necessary and sufficient that $\psi$ be injective and that, for every $x\in X$, the monomorphism $\theta^x\colon\kres(\psi(x))\to\kres(x)$ makes $\kres(x)$ a \unsure{radical} extension of $\kres(\psi(x))$.
\end{prop}

\begin{proof}
\label{proof-1.3.5.8}
We suppose that $f$ is \unsure{radical} and first show that the relation $\psi(x_1)=\psi(x_2)=y$ leads necessarily to $x_1=x_2$.
In fact, there exists a field $K$, an extension of $\kres(y)$, and $\kres(y)$-monomorphisms $\kres(x_1)\to K$ and $\kres(x_2)\to K$ (Bourbaki, {\em Alg.}, chap.~V, §4, prop.~2); the corresponding morphisms $u_1\colon\Spec(K)\to X$ and $u_2\colon\Spec(K)\to X$ are then such that $f\circ u_1=f\circ u_2$, and so $u_1=u_2$ by hypothesis, and this implies, in particular, that $x_1=x_2$.
...
Conversely, taking \hyperref[1.2.4.6]{(2.4.6)} into account, it is immediate that the conditions stated are sufficient for $f$ to be \unsure{radical}.
\end{proof}
...
\begin{proof}
\label{proof-1.3.5.9}
In fact, this morphism is a monomorphism \hyperref[1.1.6.2]{(1.6.2)}.
\end{proof}

\begin{cor}[3.5.10]
\label{1.3.5.10}
Let $f:X\to Y$ be a \unsure{radical} morphism, $g:Y'\to Y$ a morphism, and $X'=X_{(Y')}=X\times_Y Y'$.
Then the \unsure{radical} morphism $f_{(Y')}$ \hyperref[1.3.5.7]{(3.5.7~(ii))} is a bijection from the underlying space of $X$ to $g^{-1}(f(X))$; further, for every field $K$, the set $X'(K)$ can be identified with the subset of $Y'(K)$ given by the inverse image of the map $Y'(K)\to Y(K)$ (corresponding to $g$) from the subset $X(K)$ of $Y(K)$.
\end{cor}

\begin{proof}
\label{proof-1.3.5.10}
The first claim follows from \hyperref[1.3.5.8]{(3.5.8)} and \hyperref[1.3.4.8]{(3.4.8)}; the second, from the commutativity of the following diagram:\oldpage[I]{117}
\[
    \begin{tikzcd}
        X'(K) \ar[r] \ar[d]
        & Y'(K) \ar[d]\\
...
\begin{rmk}[3.5.11]
\label{1.3.5.11}
We say that a morphism $f=(\psi,\theta)$ of preschemes is {\em injective} if the map $\psi$ is injective.
For a morphism $f=(\psi,\theta):X\to Y$ to be \unsure{radical}, it is necessary and sufficient that, for every morphism $Y'\to Y$, the morphism $f_{(Y')}:X_{(Y')}\to Y'$ be injective (which justifies the terminology of a {\em universally injective} morphism).
In fact, the condition is necessary by \hyperref[1.3.5.7]{(3.5.7),~(ii)} and \hyperref[1.3.5.8]{(3.5.8)}.
...
But then, setting $Y'=\Spec(K)$, there would be two distinct $Y'$-sections of $X_{(Y')}$ \hyperref[1.3.3.14]{(3.3.14)}, which contradicts the hypothesis that $f_{(Y')}$ is injective.
\end{rmk}
...
\begin{prop}[3.6.1]
\label{1.3.6.1}
Let $f:X\to Y$ be a morphism, $y$ a point of $Y$, and $\mathfrak{a}_y$ a \unsure{defining ideal} of $\OO_y$ for the $\mathfrak{m}_y$-preadic topology.
The projection $p:X\times_Y\Spec(\OO_y/\mathfrak{a}_y)\to X$ is a homeomorphism from the underlying space of the prescheme $X\times_Y\Spec(\OO_y/\mathfrak{a}_y)$ to the fibre $f^{-1}(y)$ equipped with the topology induced from that of the underlying space of $X$.
\end{prop}

\begin{proof}
\label{proof-1.3.6.1}
Since $\Spec(\OO_y/\mathfrak{a}_y)\to Y$ is \unsure{radical} (\hyperref[1.3.5.4]{3.5.4} and \hyperref[1.2.4.7]{2.4.7}), $\Spec(\OO_y/\mathfrak{a}_y)$ is \unsure{reduced at a single point}, and the ideal $\mathfrak{m}_y/\mathfrak{a}_y$ is nilpotent by hypothesis \hyperref[1.1.1.12]{(1.1.12)}, we already know (\hyperref[1.3.5.10]{3.5.10} and \hyperref[1.3.3.4]{3.3.4}) that $p$ identifies, as {\em sets}, the underlying space of $X\times_Y\Spec(\OO_y/\mathfrak{a}_y)$ with $f^{-1}(y)$; everything boils down to proving that $p$ is a homeomorphism.
By \hyperref[1.3.2.7]{(3.2.7)}, the question is local on $X$ and $Y$, and so we can suppose that $X=\Spec(B)$ and $Y=\Spec(A)$, with $B$ being an $A$-algebra.
...
\]
where $s\not\in\mathfrak{j}_y$, and prop.~\hyperref[1.1.2.4]{(1.2.4)} applies.
\end{proof}

\begin{env}[3.6.2]
\label{1.3.6.2}
Throughout the rest of this treatise, whenever we consider a fibre $f^{-1}(y)$ of a morphism as having the structure of a $\kres(y)$-prescheme, {\em it will always be the prescheme obtained by transporting the structure of $X\times_Y\Spec(\kres(y))$ by the projection to $X$}.
...
With the preceding convention, it follows from \hyperref[1.3.5.10]{(3.5.10)} that the points of $X$ with values in an extension $K$ of $\kres(y)$ are identified with the {\em points of $f^{-1}(y)$ with values in $K$}.
...
\begin{prop}[3.6.5]
\label{1.3.6.5}
Let $f:X\to Y$ be a morphism, $y$ a point of $Y$, $Z$ the local prescheme $\Spec(\OO_y)$, and $p=(\psi,\theta)$ the projection $X\times_Y Z\to X$; then $p$ is a homeomorphism from the underlying space of $X\times_Y Z$ to the subspace $f^{-1}(Z)$ of $X$ ({\em when the underlying space of $Z$ is identified with a subspace of $Y$, cf.~\hyperref[1.2.4.2]{(2.4.2)}}), and, for all $t\in X\times_Y Z$, letting $z=\psi(t)$, $\theta_t^\#$ is an isomorphism from $\OO_x$ to $\OO_t$.
\end{prop}

\begin{proof}
\label{proof-1.3.6.5}
Since $Z$ (identified as a subspace of $Y$) is contained inside every open affine containing $y$ \hyperref[1.2.4.2]{(2.4.2)}, we can, as in \hyperref[1.3.6.1]{(3.6.1)}, reduce to the case where $X=\Spec(A)$ and $Y=\Spec(B)$ are affine schemes, with $A$ being a $B$-algebra.
Then $X\times_Y Z$ is the prime spectrum of $A\otimes_B B_y$, and this ring is canonically identified with $S^{-1}A$, where $S$ is the image of $B\setminus\mathfrak{j}_y$ in $A$ \hyperref[0.1.5.2]{(\textbf{0},~1.5.2)}; since $p$ then corresponds to the canonical homomorphism $A\to S^{-1}A$, the proposition follows from \hyperref[1.1.6.2]{(1.6.2)}.
\end{proof}

\subsection{Application: reduction of a prescheme modulo $\mathfrak{J}$}

{\em This section, which makes use of notions and results from Chapter~I and Chapter~II, will not be used in the following, and is only intended for readers familiar with classical algebraic geometry}.

\begin{env}[3.7.1]
\label{1.3.7.1}
Let $A$ be a ring, $X$ an $A$-prescheme, and $\mathfrak{J}$ an ideal of $A$; then $X_0=X\otimes_A(A/\mathfrak{J})$ is an $(A/\mathfrak{J})$-prescheme, which we sometimes say is \unsure{inferred} from $X$ by {\em reduction} modulo $\mathfrak{J}$.
\end{env}

\begin{env}
\label{1.3.7.2}
This terminology is used foremost when $A$ is a {\em local ring} and $\mathfrak{J}$ its maximal ideal, in such a way that $X_0$ is a prescheme over the residue field $k=A/\mathfrak{J}$ of $A$.

When $A$ is also integral, with field of fractions $K$, we can consider the $K$-prescheme $X'=X\otimes_A K$.
By an abuse of language which we will not use, it has been said, up until now, that $X_0$ is \unsure{{\em induced by $X'$}} by reduction modulo $\mathfrak{J}$.
In the case where this language was used, $A$ was a local ring of dimension~$1$ (most often a discrete valuation ring) and it was implied (be it more or less explicitly) that the given $K$-prescheme $X'$ was a closed subprescheme of a $K$-prescheme $P'$ (in fact, a projective space \unsure{of the form} $\bb{P}_K^r$, cf.~\textbf{II},~4.1.1), itself of the form $P'=P\otimes_A K$, where $P$ is a given $A$-prescheme (in fact, the $A$-scheme $\bb{P}_A^r$, with the notation of \textbf{II},~4.1.1).
...
\oldpage[I]{119}
$y=\mathfrak{J}$ and the generic point $(0)$, the set $U$ \unsure{reduced to} the generic point being thus open $U=\Spec(K)$ in $Y$.
...
If $P$ is Noetherian (for example, if $A$ is Noetherian and $P$ is of finite type over $A$), then there exists a smaller closed subprescheme $X=\overline{X'}$ of $G$ that \unsure{contains} $X$ \hyperref[1.9.5.10]{(9.5.10)}, and $X'$ is the prescheme induced by $X$ on the open $\vphi^{-1}(U)\cap X$, and so is isomorphic to $X\otimes_A K$ \hyperref[1.9.5.10]{(9.5.10)}.
{\em The immersion of $X'$ into $P'=P\otimes_A K$ thus lets us canonically consider $X'$ as being of the form $X'=X\otimes_A K$, where $X$ is an $A$-prescheme.}
We can then consider the reduced modulo $\mathfrak{J}$ prescheme $X_0=X\otimes_A k$, which is nothing but the fibre $\psi^{-1}(y)$ of the closed point $y$.
Up until now, lacking the adequate terminology, we had avoided explicitly introducing the $A$-prescheme $X$.
One ought to, however, note that all the assertions normally made about the ``reduced modulo $\mathfrak{J}$'' prescheme  $X_0$ should be seen as consequences of more complicated assertions concerning $X$ itself, and cannot be satisfactorily formulated or understood except by interpreting them as such.
...
\begin{env}[3.7.3]
\label{1.3.7.3}
Lastly, we draw attention to a very particular fact, which has undoubtedly contributed to slowing the conceptual clarification of the situation envisaged here: if $A$ is a discrete valuation ring, and if $X$ is {\em proper} over $A$ (which is indeed the case if $X$ is a closed subprescheme of some $\bb{P}_A^r$, cf.~\textbf{II},~5.5.4), then the points of $X$ with values in $A$ and the points of $X'$ with values in $k$ are in bijective correspondence (\textbf{II},~7.3.8).
