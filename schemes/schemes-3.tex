...
\begin{prop}[3.5.8]
\label{1.3.5.8}
For a morphism $f=(\psi,\theta):X\to Y$ to be radical, it is necessary and sufficient that $\psi$ be injective and that, for every $x\in X$, the monomorphism $\theta^x:\kres(\psi(x))\to\kres(x)$ makes $\kres(x)$ a radical extension of $\kres(\psi(x))$.
\end{prop}
...
Conversely, the condition implies that $\psi$ is injective; if for some $x\in X$ the monomorphism $\theta^x:\kres(\psi(x))\to\kres(x)$ were not radical, then there would be an extension $K$ of $\kres(\psi(x))$ and two distinct morphisms $\Spec(K)\to X$ corresponding to the same morphism $\Spec(K)\to Y$ \sref{1.3.5.8}.
