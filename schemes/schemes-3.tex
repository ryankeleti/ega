...
\begin{prop}[3.5.3]
\label{prop-1.3.5.3}
For a morphism $f:X\to Y$ to be surjective, it is necessary and sufficient that, for every field $K$ and every morphism $\Spec(K)\to Y$, there exist an extension $K'$ of $K$ and a morphism $\Spec(K')\to X$ making the following diagram commute:
\[
    \begin{tikzcd}
        X \ar[d, swap, "f"]
        & \Spec(K') \ar[l] \ar[d]\\
...
We consider the affine scheme $Y=\Spec(A)$, formed of two points, the unique closed point\oldpage{119} $y=\mathfrak{J}$ and the generic point $(0)$, the set $U$ \unsure{reduced to} the generic point being thus open $U=\Spec(K)$ in $Y$.
