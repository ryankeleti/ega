...
\begin{env}[3.2.6.5]
\label{1.3.2.6.5}
We can now complete the proof of Theorem \sref{1.3.2.6}. If $S$ is an {\em affine
...
\begin{env}[3.2.9]
\label{1.3.2.9}
\footnote{[Trans] \sref{1.3.2.9} is from the errata of EGA~II, on page 221.}
\oldpage[II]{221}
...
\begin{env}[3.3.14]
\label{1.3.3.14}
With the notation of \sref{1.3.3.6}, for each {\em $S$-morphism}
...
it follows in particular from \sref{1.3.3.14} that the {\em $X$-sections} of
...
If $i\colon\Spec(\kres(x))\to X$ and $j\colon\Spec(\kres(y))\to Y$ are the canonical morphisms \sref{1.2.4.5}, we take for $\alpha$ the map of underlying spaces corresponding to the morphism $i\times_S j$.
...
\begin{proof}
\label{proof-1.3.5.2}
The composition of two surjections being a surjection, it suffices to prove (ii); but this proposition follows quickly from \sref{1.3.4.8} applied to $M=X$.
\end{proof}
...
With the language introduced in \sref{1.3.4.5}, we can say that {\em every geometric point of $Y$ with values in $K$ comes from a geometric point of $X$ with values in an extension of $K$}.
...
\begin{env}[3.5.5]
\label{1.3.5.5}
For a morphism $f:X\to Y$ to be \unsure{radical}, it suffices that the condition of def.~\sref{1.3.5.4} hold for every {\em algebraically closed} field.
...
Conversely, taking \sref{1.2.4.6} into account, it is immediate that the conditions stated are sufficient for $f$ to be \unsure{radical}.
\end{proof}
...
\begin{proof}
\label{proof-1.3.5.9}
In fact, this morphism is a monomorphism \sref{1.1.6.2}.
\end{proof}
...
In fact, the condition is necessary by \hyperref[1.3.5.7]{(3.5.7),~(ii)} and \sref{1.3.5.8}.
Conversely, the condition implies that $\psi$ is injective; if for some $x\in X$ the monomorphism $\theta^x\colon\kres(\psi(x))\to\kres(x)$ were not \unsure{radical}, then there would be an extension $K$ of $\kres(\psi(x))$ and two distinct morphisms $\Spec(K)\to X$ corresponding to the same morphism $\Spec(K)\to Y$ \sref{1.3.5.8}.
...
\]
where $s\not\in\mathfrak{j}_y$, and prop.~\sref{1.1.2.4} applies.
\end{proof}
...
With the preceding convention, it follows from \sref{1.3.5.10} that the points of $X$ with values in an extension $K$ of $\kres(y)$ are identified with the {\em points of $f^{-1}(y)$ with values in $K$}.
...
\begin{prop}[3.6.5]
\label{1.3.6.5}
Let $f:X\to Y$ be a morphism, $y$ a point of $Y$, $Z$ the local prescheme $\Spec(\OO_y)$, and $p=(\psi,\theta)$ the projection $X\times_Y Z\to X$; then $p$ is a homeomorphism from the underlying space of $X\times_Y Z$ to the subspace $f^{-1}(Z)$ of $X$ ({\em when the underlying space of $Z$ is identified with a subspace of $Y$, cf.~\sref{1.2.4.2}}), and, for all $t\in X\times_Y Z$, letting $z=\psi(t)$, $\theta_t^\#$ is an isomorphism from $\OO_x$ to $\OO_t$.
\end{prop}

\begin{proof}
\label{proof-1.3.6.5}
Since $Z$ (identified as a subspace of $Y$) is contained inside every open affine containing $y$ \sref{1.2.4.2}, we can, as in \sref{1.3.6.1}, reduce to the case where $X=\Spec(A)$ and $Y=\Spec(B)$ are affine schemes, with $A$ being a $B$-algebra.
Then $X\times_Y Z$ is the prime spectrum of $A\otimes_B B_y$, and this ring is canonically identified with $S^{-1}A$, where $S$ is the image of $B\setminus\mathfrak{j}_y$ in $A$ \hyperref[0.1.5.2]{(\textbf{0},~1.5.2)}; since $p$ then corresponds to the canonical homomorphism $A\to S^{-1}A$, the proposition follows from \sref{1.1.6.2}.
\end{proof}
