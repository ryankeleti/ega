...
The existence and the properties of $\sh{T}$ follow immediately from Propositions \sref{1.10.3.4} and \sref{1.5.1.1}, taking into account the existence and the properties of the largest ideal of definition for $A$ (\sref[0]{0.7.1.6} and \sref[0]{0.7.1.7}).
...
\begin{enumerate}[label=(\roman*)]
  \item If $f=(\psi,\theta)$, then the hypotheses imply that the image under $\theta^\sharp:\psi^*(\OO_\fk{Y})\to\OO_\fk{X}$ of the sheaf of ideals $\psi^*(\sh{K})$ of $\psi^*(\OO_\fk{Y})$ is contained in $\sh{J}$ \sref[0]{0.4.3.5}.
    ...
    \[
      \omega:\psi^*(\OO_\fk{Y}/\sh{K})=\psi^*(\OO_\fk{Y})/\psi^*(\sh{K})\longrightarrow\OO_\fk{X}/\sh{J};
    \]
