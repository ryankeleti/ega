...
\begin{prop}[5.1.4]
\label{1.5.1.4}
For the prime spectrum of a ring $A$ to be a reduced (\emph{resp.} integral) prescheme \sref{1.2.1.7}, it is necessary and sufficient that $A$ is a reduced (\emph{resp.} integral) ring.
\end{prop}
...
\[
  \omega:\psi^*(\OO_Y/\sh{N}_Y)\longrightarrow\OO_X/\sh{N}_X.
\]
...
for $X$ to be an affine scheme, it is necessary and sufficient that $X_0$ to be an affine scheme.
\end{prop}
