...
\begin{proof}
\label{proof-1.7.2.5}
We\oldpage[I]{160} have to show that $p\circ\overline{f}$ is the identity on the domain of definition of $\overline{f}$; since $X$ is separated over $S$, this again follows from \hyperref[1.7.2.2.1]{(7.2.2.1)}.
\end{proof}
...
Let $f$ be a rational $S$-map from $X$ to $Y$, and let $x$ be a point of $X$; we can compose $f$ with the canonical $S$-morphism $\Spec(\OO_x)\to X$ \hyperref[1.2.4.1]{(2.4.1)} provided that the \unsure{trace on} $\Spec(\OO_x)$ of the domain of definition of $f$ is dense in $\Spec(\OO_x)$ (identified with the set of $z\in X$ such that $x\in\overline{\{z\}}$ \hyperref[1.2.4.2]{(2.4.2)}).
...
The irreducible components of $\Spec(\OO_x)$ are the \unsure{traces on} $\Spec(\OO_x)$ of the irreducible components of $X$ containing $x$.
...
Since $\Spec(\OO_x)$ is contained in every affine open subset $U$ that contains $x$, and since the irreducible components of $U$ that contain $x$ are the \unsure{traces on} $U$ of the irreducible components of $X$ containing $x$ \hyperref[0.2.1.6]{(\textbf{0},~2.1.6)}, we can suppose that $X$ is affine, given by a ring $A$.
Since the prime ideals of $A_x$ correspond bijectively to the prime ideals of $A$ that are\oldpage[I]{161} contained in $\mathfrak{j}_x$ \hyperref[0.1.2.6]{(\textbf{0},~2.1.6)}, the minimal prime ideals of $A_x$ correspond to the minimal prime ideals of $A$ that are contained in $\mathfrak{j}_x$, whence the lemma.
\end{proof}

That being said, suppose that we are in one of the two cases mentioned above.
If $U$ is the domain of definition of the rational $S$-map $f$, we denote by $f'$ the rational map from $\Spec(\OO_x)$ to $Y$ which agrees (taking \hyperref[1.2.4.2]{(2.4.2)} into account) with $f$ on $U\cap\Spec(\OO_x)$; we say that this rational map is \emph{induced by $f$}.
...
For $f$ to be defined at a point $x$, it is necessary and sufficent that the rational map $f'$ from $\Spec(\OO_x)$ to $Y$, induced by $f$ \hyperref[1.7.2.8]{(7.2.8)}, be a morphism.
\end{prop}
...
By \hyperref[1.6.5.1]{(6.5.1)}, there exists an open neighbourhood $U$ of $x$ in $X$, and an $S$-morphism $g$ from $U$ to $Y$, inducing $f'$ on $\Spec(\OO_x)$.
If $X$ is irreducible, then $U$ is dense in $X$, and by \hyperref[1.7.2.3]{(7.2.3)} we can suppose that $g$ is a rational $S$-map.
Further, the generic point of $X$ belongs to $\Spec(\OO_x)$ and to the domain of definition of $f$, so $s$ and $g$ agree at this point, and thus on a non-empty open subset of $X$ \hyperref[1.6.5.1]{(6.5.1)}.
But since $f$ and $g$ are rational $S$-maps, they are identical \hyperref[1.7.2.3]{(7.2.3)}, and so $f$ is defined at $x$.

If we now suppose that $X$ is locally Noetherian, we can suppose that $U$ is Noetherian; then there are only a finite number of irreducible components $X_i$ of $X$ that contain $x$ \hyperref[1.7.2.8.1]{(7.2.8.1)}, and we can suppose that they are the only ones that have a non-empty intersection with $U$, by replacing, if needed, $U$ with a smaller open subset (since there are only a finite number of irreducible components of $X$ that have a non-empty intersection with $U$, $U$ being Noetherian).
...
Since $U\cup(X\setminus\overline{U})$ is dense in $X$, $f_1$ and $f$ agree on a dense open subset of $X$, and since $f$ is a rational map, $f$ is an extension of $f_1$ \hyperref[1.7.2.3]{(7.2.3)}, and is thus defined at the point $x$.
\end{proof}
...
For every open subset $U\subset X$, we denote by $R(U)$ the ring of rational functions on $U$ \hyperref[1.7.1.3]{(7.1.3)}; it is a $\Gamma(U,\OO_X)$-algebra.
...
We thus define a \unsure{dihomomorphism} of algebras $\rho_{V,U}\colon R(U)\to R(V)$, and it is clear that if $U\supset V\supset W$ are three open subsets of $X$, then we have $\rho_{W,U}=\rho_{W,V}\circ\rho_{V,U}$; the $R(U)$ thus define a \emph{presheaf} of algebras on $X$.
\end{env}
...
Then the $\OO_X$-module $\sh{R}(X)$ is quasi-coherent, and for every open subset $U$ of $X$ that has a non-empty intersection with only finitely many of the components $X_\lambda$, $R(U)$ is equal to $\Gamma(U,\sh{R}(X))$, and can be canonically identified with the \unsure{direct ???} of the local rings of the generic points of the $X_\lambda$ such that $U\cap X_\lambda\neq\emp$.
\end{prop}
...
The fact that $R(U)$ can be canonically identified with the \unsure{direct ???} of the $\OO_{x_i}=R(X_i)$ such that $U\cap X_i\neq\emp$ then follows from \hyperref[1.7.1.7]{(7.1.7)}.
...
Finally, to see that $\sh{R}(X)$ is quasi-coherent, we can restrict to the case where $X=\Spec(A)$ is affine; by taking $U$ to be an affine open subset of the form $D(f)$, where $f\in A$, it follows from the above and from definition~\hyperref[1.1.3.4]{(1.3.4)} that we have $\sh{R}(X)=\widetilde{M}$, where $M$ is the direct sum of the $A$-modules $A_{x_i}$.
\end{proof}

\begin{cor}[7.3.4]
\label{1.7.3.4}
Let $X$ be a reduced prescheme that has only a finite number of irreducible components, and let $X_i$ ($1\leqslant i\leqslant n$) be the closed reduced preschemes of $X$ that have the irreducible components of $X$ as their underlying spaces \hyperref[1.5.2.1]{(5.2.1)}.
If $h_i$ is the canonical injection $X_i\to X$, then $\sh{R}(X)$ is then the \unsure{direct ???} of the $\OO_X$-algebras $(h_i)_*(\sh{R}(X_i))$.
\end{cor}
...
\begin{proof}
\label{proof-1.7.3.5}
It suffices to show that every $x\in X$ admits a neighbourhood $U$ such that $\sh{F}|U$ is a simple sheaf \hyperref[0.3.6.2]{(\textbf{0},~3.6.2)}, or, in other words, we are led to considering the case where $X$ is affine; we can further suppose that $\sh{F}$ is the cokernel of a homomorphism $(\sh{R}(X))^{I}\to(\sh{R}(X))^{J}$ \hyperref[0.5.1.3]{(\textbf{0},~5.1.3)}, and everything then follows from showing that $\sh{R}(X)$ is a simple sheaf; but this is evident, because $\Gamma(U,\sh{R}(X))=R(X)$ for every non-empty open subset $U$, where $U$ contains the generic point of $X$.
\end{proof}
...
\begin{proof}
\label{proof-1.7.3.7}
The question being local, the first claim follows from \hyperref[1.7.3.3]{(7.3.3)}; the second follows from \hyperref[1.7.2.3]{(7.2.3)}.
\end{proof}
...
\begin{proof}
\label{proof-1.7.3.8}
Indeed, we are led (by \hyperref[1.7.3.3]{(7.3.3)}) to the case where $X$ and $Y$ are irreducible, with generic points $x$ and $y$, with $f(x)=y$; thus $(f^*(\sh{R}(Y)))_x = \OO_y\otimes_{\OO_y}\OO_x = \OO_x$ \hyperref[0.4.3.1]{(\textbf{0},~4.3.1)}, which proves \hyperref[1.7.3.8]{(7.3.8.1)}, thanks to \hyperref[1.7.3.5]{(7.3.5)}.
\end{proof}
...
The \emph{kernel $\sh{T}$} of this homomorphism is a sub-$\OO_X$-module of $\sh{F}$, called the \emph{torsion sheaf} of $\sh{F}$; it is quasi-coherent if $\sh{F}$ is quasi-coherent (\hyperref[1.4.1.1]{4.1.1} and \hyperref[1.7.3.6]{7.3.6}).
...
We deduce from \hyperref[1.7.3.5]{(7.3.5)} that:
\end{env}
...
Since $\sh{L}_x$ and $\sh{L}'_x$ can be identified with sub-$\OO_X$-modules of the field $\OO_x$, the above equation leads to $f_x=0$ or $f'_x=0$, and thus $f=0$ or $f'=0$, since $\sh{L}$ and $\sh{L}'$ are torsion free \hyperref[1.7.3.5]{(7.3.5)}.
\end{proof}
...
\begin{proof}
\label{proof-1.7.4.5}
Since\oldpage[I]{164} $f_*$ is left exact \hyperref[0.4.2.1]{(\textbf{0},~4.2.1)}, it suffices, by \hyperref[1.7.4.2]{(7.4.2)}, to prove the proposition when $\sh{F}=(\sh{R}(X))^{(I)}$.
But every non-empty open subset $U$ of $Y$ contains the generic point of $Y$, so $f^{-1}(U)$ contains the generic point of $X$ \hyperref[0.2.1.5]{(\textbf{0},~2.1.5)}, so we have that $\Gamma(U,f_*(\sh{F})) = \Gamma(f^{-1}(U),\sh{F}) = (R(X))^{(I)}$; in other words, $f_*(\sh{F})$ is the simple sheaf with fibre $(R(X))^{(I)}$, considered as a $\sh{R}(Y)$-module, and it is clearly torsion free.
\end{proof}
...
\begin{proof}
\label{proof-1.7.4.6}
By \hyperref[1.7.3.5]{(7.3.5)} and \hyperref[1.7.4.1]{(7.4.1)}, the equations $\sh{F}_x=0$ and $\sh{F}\otimes_{\OO_X}\sh{R}(X)=0$ are equivalent, so \emph{a)} and \emph{b)} are equivalent; also, $\Supp(\sh{F})$ is closed in $X$ \hyperref[0.5.2.2]{(\textbf{0},~5.2.2)}, and since every non-empty open subset of $X$ contains $x$, \emph{b)} and \emph{c)} are equivalent.
\end{proof}

\begin{env}[7.4.7]
\label{1.7.4.7}
We generalise (by an abuse of language) the definitions of \hyperref[1.7.4.1]{(7.4.1)} to the case where $X$ is a \emph{reduced} prescheme having only a \emph{finite} number of irreducible components; it then follows from \hyperref[1.7.3.4]{(7.3.4)} that the equivalence between \emph{a)} and \emph{c)} in \hyperref[1.7.4.6]{(7.4.6)} still holds true for such a prescheme.
\end{env}
