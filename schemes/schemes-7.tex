...
\begin{proof}
\label{proof-1.7.1.9}
Indeed, suppose that the above lemma is true; identifying the ring of sections $\Gamma(D(f),\OO_X)$ with $A_f$ (\hyperref[1.1.3.6]{1.3.6} and \hyperref[1.1.3.7]{1.3.7}), it follows from the fact that the $D(f)$ with $f\in Q$ form a \unsure{cofinal} set in the ordered (under $\supset$) set of dense open subsets of $X$, and from def.~\sref{1.7.1.1} that the ring of rational functions on $X$ can be identified with the inductive limit of the $A_f$ for $f\in Q$ (for the preorder relation ``$g$ is a multiple of $f$''), i.e. $Q^{-1}A$ \hyperref[0.1.4.5]{(\textbf{0},~1.4.5)}.
\end{proof}

\begin{proof}
\label{proof-1.7.1.9.1}
To show \sref{1.7.1.9.1}, we again denote by $X_i$ ($1\leqslant i\leqslant n$) the irreducible components of $X$; if $D(f)$ is dense in $X$ then $D(f)\cap X_i\neq\emp$ for $1\leqslant i\leqslant n$, and vice-versa; but this means that $f\not\in\mathfrak{p}_i$ for $1\leqslant i\leqslant n$, where we set $\mathfrak{p}_i=\mathfrak{j}(X_i)$, and since the $\mathfrak{p}_i$ are the minimal prime ideals of $A$ \sref{1.1.1.14}, the conditions $f\not\in\mathfrak{p}_i$ ($1\leqslant i\leqslant n$) are equivalent to $f\in Q$, whence the first claim of the lemma.
...
\begin{proof}
\label{proof-1.7.2.6}
Taking \sref{1.7.2.3}, \sref{1.7.1.2}, and \sref{1.7.1.3} into account, it suffices to note that the $X$-prescheme $X\otimes_{\bb{Z}}\bb{Z}[T]$ is separated over $X$ \hyperref[1.5.5.1]{(5.5.1,~(iv))}.
\end{proof}
...
\begin{proof}
\label{proof-1.7.2.7}
The restriction of $f$ to $f^{-1}(U)$ is a separated morphism \hyperref[1.5.5.1]{(5.5.1,~(i))}, so $g$ is a closed immersion \sref{1.5.4.6}, and so $g(U)=Z\cap f^{-1}(U)$, and the subprescheme induced by $Z$ on the open subset $g(U)$ of $Z$ is identical to the closed subprescheme of $f^{-1}(U)$ associated to $g$ \sref{1.5.2.1}.
...
\begin{proof}
\label{proof-1.7.3.8}
Indeed, we are led (by \sref{1.7.3.3}) to the case where $X$ and $Y$ are irreducible, with generic points $x$ and $y$, with $f(x)=y$; thus $(f^*(\sh{R}(Y)))_x = \OO_y\otimes_{\OO_y}\OO_x = \OO_x$ \hyperref[0.4.3.1]{(\textbf{0},~4.3.1)}, which proves \hyperref[1.7.3.8]{(7.3.8.1)}, thanks to \sref{1.7.3.5}.
\end{proof}
...
\begin{proof}
\label{proof-1.7.4.5}
Since\oldpage[I]{164} $f_*$ is left exact \hyperref[0.4.2.1]{(\textbf{0},~4.2.1)}, it suffices, by \sref{1.7.4.2}, to prove the proposition when $\sh{F}=(\sh{R}(X))^{(I)}$.
...
\begin{proof}
\label{proof-1.7.4.6}
By \sref{1.7.3.5} and \sref{1.7.4.1}, the equations $\sh{F}_x=0$ and $\sh{F}\otimes_{\OO_X}\sh{R}(X)=0$ are equivalent, so \emph{a)} and \emph{b)} are equivalent; also, $\Supp(\sh{F})$ is closed in $X$ \hyperref[0.5.2.2]{(\textbf{0},~5.2.2)}, and since every non-empty open subset of $X$ contains $x$, \emph{b)} and \emph{c)} are equivalent.
\end{proof}
