...
\begin{enumerate}[label=\emph{(\roman*)}]
  ...
  \item If the hypotheses of Proposition \sref{1.9.5.1} are satisfied and $X$ is irreducible (\emph{resp.} \unsure{integral}), then so too is $Y'$.
\end{enumerate}
\end{prop}
...
As $X$ is reduced, $g$ factors as $X\xrightarrow{h} {Y'}_\text{red}\xrightarrow{j'}Y'$, where $j'$ is the canonical injection \sref{1.5.2.2}, and it then follows from the definition of $Y'$ that $Y'_\text{red}=Y'$.
