...
\begin{env}[11.4.2]
\label{0.11.4.2}
Let $K^\bullet = (K^i)_{i\in Z}$ be a complex of objects of $\cat{C}$, whose differential operator is of degree~$+1$.
...
where $\RR^p T$ denotes, as usual, the $p$-th \emph{derived functor} of $T$ for $p\in Z$;
$\RR^q T(K^\bullet)$ denotes the complexes $(\RR^q T(K^i))_{i\in Z}$.
...
If we assume that filtrant inductive limits exist and are exact in $\cat{C}'$ (which implies, in particular, the existence of infinite direct sums in $\cat{C}'$), then the complex defined by the bicomplex $T(L^{\bullet\bullet})$ exists, and criterion \sref{0.11.3.3} shows that the sequence $'E(T(L^{\bullet\bullet}))$ is always \emph{regular}.
...
\label{0.11.4.5}
Suppose that in $\cat{C}'$ the inductive limit filters exist and are exact.
If $\RR^n T(K^i) = 0$ for all $n>0$ and all $i\in Z$, we have functorial isomorphisms
\[
\label{0.11.4.5.1}
  \RR^i T(K^\bullet) \simto \HH^i(T(K^\bullet)), (i\in Z)
\tag{11.4.5.1}
\]
\end{proposition}
Indeed, the only non-zero $E_2$ terms of the first spectral sequence \sref{0.11.4.3.1} are then $'E_2^{p0} = \HH^p(T(K^\bullet))$;
In other words, this sequence is \emph{degenerate}; as it is regular \sref{0.11.4.4}, the conclusion follows from \sref{0.11.1.6}.


\begin{env}[11.4.6]
\label{0.11.4.6}
Now consider, for example, a covariant bifunctor $(M, N) \rightarrow T(M, N)$ of $\cat{C} \times \cat{C}$ in $\cat{C}''$.
$\cat{C}, \cat{C}', \cat{C}''$ are three abelian category; We assume, for simplicity, $T$ additive in each of its argument, 
and furthermore that all object of $\cat{C}$ and all object of $\cat{C}'$ are subobjects of an injective object, 
and that the inductive limit filters exist in $\cat{C}$ and are exact.
We then define \emph{hypercohomology} of $T$ with respect to two complexes $K^\bullet, K'^\bullet$ of $\cat{C}$ and $\cat{C}'$ respectively, 
with differential operators of degree~$+1$, having $K^\bullet$ (resp. $K'^\bullet$) a injective Cartan--Eilenberg resolution $L^{\bullet\bullet}$ (resp. $\bcx{L'}$);
then $T(L^{\bullet\bullet}, \bcx{L'})$ is a quadricomplex of $\cat{C}''$, which we consider as a \emph{bicomplex} of $\cat{C}'$ having degree of $T(L^{ij}, L'^{hk})$ the integers $i+h$ and $j+k$.
The \emph{hypercohomology} of $T$ with respect to $K^\bullet$ and $K'^\bullet$ is by definitoin the cohomology $\HH^\bullet(T(L^{\bullet\bullet}, \bcx{L'}))$ of this bicomplex 
(in other words, that of the associated simple complex) denoted $\RR^\bullet T(K^\bullet, K'^\bullet)$;
\oldpage[0\textsubscript{III}]{35}
It is the abutment of two spectral sequences whose terms $E_2$ are given by
\[
\label{0.11.4.6.1}
  'E_2^{pq} = \HH^p(\RR^q T(K^\bullet, K'^\bullet))
\tag{11.4.6.1}
\]
\[
\label{0.11.4.6.2}
  ''E_2^{pq} = \sum_{q'+q''=q}\RR^p T(\HH^{q'}{K^\bullet},\HH^{q''}{K'^\bullet}), (cf.\ M, XVII, 2).
\tag{11.4.6.2}
\]

Here $\RR^q T(K^\bullet, K'^\bullet)$ is the bicomplex $(\RR^q T(K^i, K'^j))_{(i,j)\in Z\times Z}$ and the second member of \sref{0.11.4.6.1} is its cohomology when we consider it as a simple complex.
...
admit an injective resolution of length $\leq n$, or when $K^\bullet$ and $K'^\bullet$ are inf limit;
In the latter case, we can furthermore omit the hypothesis that the inductive limits exist in $\cat{C}$ and $\cat{C}'$.

If $K^\bullet, K'^\bullet$ are two other complexes of $\cat{C}$ and $\cat{C}'$ respectively.
$f, g$ two morphism \emph{homotopes} from $K^\bullet$ to $\cx{K_1}$, 
$f', g'$ two morphism \emph{homotopes} from $K'^\bullet$ to $\cx{K'_1}$.
Then the morphisms $\RR^\bullet  T(K^\bullet, K'^\bullet) \rightarrow \RR^\bullet  T(\cx{K_1}, \cx{K'_1})$ 
...
\label{0.11.4.7}
Suppose that for any injective object $\cat{I}$ of $\cat{C}$ (resp. $\cat{I'}$ of $\cat{C}'$), $\cat{A'}\rightsquigarrow \cat{T}(\cat{I}, \cat{A'})$ (resp. $\cat{A}\rightsquigarrow \cat{T}(\cat{A}, \cat{I'})$)
is an exact functor. Then, with the notation of \sref{0.11.4.6}, we have a canonical isomorphism
\[
  \RR^\bullet T(K^\bullet, K'^\bullet) \simto \HH^\bullet(T(L^{\bullet\bullet}, K'^\bullet)) \simto \HH^\bullet(T(K^\bullet, \bcx{L'})) 
\]
where the last two terms are the cohomology of simple complexes defined by tricomplexes $T(L^{\bullet\bullet}, K'^\bullet)$ and $T(K^\bullet, L^{\bullet\bullet})$ respectively.
\end{proposition}

Let us define for example the first of these isomorphisms. The quadricomplex $T(L^{\bullet\bullet}, \bcx{L'})$ can be considered as a bicomplex, having degree of $T(L^{ij}, L'^{hk})$ the numbers $i+j$ and $h+k$.
For each $h, \cx{L'^h}$ is a \emph{resolution} of $K'^h$, we have, for this bicomplex, by virtue of the hypothesis on $\cat{T}, \HH_\mathrm{II}^q(T(L^{\bullet\bullet}, \bcx{L'}))=0$ for $q\neq 0$ and $\HH_\mathrm{II}^0(T(L^{\bullet\bullet}, \bcx{L'})) = T(L^{\bullet\bullet}, K'^\bullet)$; 
The first spectral sequence of this bicomplex is therefore \emph{degenerate}; As $L'^{hk}=0$ for $k<0$, this sequence is also regular \sref{0.11.3.3} and the conclusion therefore results from \sref{0.11.1.6}.
