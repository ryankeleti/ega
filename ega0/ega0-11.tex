...
Suppose that the (simple) complex defined by the bicomplex $T(\bcx{L})$ exists \sref{0.11.3.1};
...
By virtue of \sref{0.11.4.2} and \sref{0.11.4.3}, they only depend on complex $\cx{K}$ effectively;
...
As $L^{ij}=0$ for $j<0$, the criteria of \sref{0.11.3.3} shows that the \emph{two} hypercohomology of spectral sequences of $T$ with respect to $\cx{K}$ exist 
...
Indeed, in the first case, we can suppose \sref{0.11.4.2} that there exists $i_0$ such that $L^{ij}=0$ for $i<i_0$.
...
Then the complex defined by the bicomplex $T(\bcx{L})$ exists, and criteion \sref{0.11.3.3} shows that the sequence $'E(T(\bcx{L}))$ is always \emph{regular}.
...
\]
\end{proposition}
Indeed, the only non-zero $E_2$ terms of the first spectral sequence \sref{0.11.4.3.1} are then $'E_2^{p0} = \HH^p(T(\cx{K}))$;
...
Here $\RR^q T(\cx{K}, \cx{K'})$ is the bicomplex $(\RR^q T(K^i, K'^j))_{(i,j)\in Z\times Z}$ and the second member of \sref{0.11.4.6.1} is its cohomology when we consider it as a simple complex.
