...
\begin{enumerate}[label=\emph{(\alph*)}]
  \item For $i\leq j$, $u_{ij}$ is surjective and its kernel is $\mathfrak{J}_j^{i+1}$
...
\begin{enumerate}[label=\emph{(\roman*)}]
  \item $A$ is an adic ring, having $\mathfrak{J}=\mathfrak{J}^{(1)}$ for an
    ideal of definition.
  \item We have $\mathfrak{J}^{(n)}=\mathfrak{J}^n$ for each $n\geq 1$.
...
\begin{enumerate}
  ...
  \item[(ii)] Set $A_i=A/\mathfrak{J}^{i+1}$, and for $i\leq j$, let $u_{ij}$ be the
    ...
    is $A'/\mathfrak{J}'=(A/\mathfrak{J})[T_1,\dots,T_r]$, hence $A'$ is Notherian \sref{0.7.2.8}.
\end{enumerate}
\end{proof}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Set $A_i=A/\mathfrak{J}^{i+1}$, and for $i\leq j$, let $u_{ij}$ be the canonical
    ...
    Noetherian, then so is $A_0'=S_0^{-1}(A/\mathfrak{J})$, which finishes the proof \sref{0.7.2.8}.
\end{enumerate}
\end{proof}
...
topology of $\Hom_A(L,N)$ is the $\mathfrak{J}$-adic \sref{0.7.3.2}, we have
