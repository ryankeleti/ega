...
\label{eq:0.14.3.3.3}
  ...
  \tag{14.3.3.3}
\]
from \eref{eq:0.14.3.2.1}, we immediately deduce \eref{eq:0.14.3.3.1} and \eref{eq:0.14.3.3.2} from \eref{eq:0.14.3.3.3}.
Conversely, \eref{eq:0.14.3.3.1} implies \eref{eq:0.14.3.2.1}, and so \eref{eq:0.14.3.3.1} implies the chain condition, by \sref{0.14.3.2};
further, by applying \eref{eq:0.14.3.3.1} to the case where $Y$ is a single closed point $x$ of $X$, and $Z$ is an irreducible component of $X$, we get that $\codim(\{x\},X)=\dim(Z)$;
we thus conclude that (c) implies (b).
Similarly, \eref{eq:0.14.3.3.2} implies \eref{eq:0.14.3.2.1}, and thus the chain condition;
further, with the same choice of $Y$ and $Z$ as above, \eref{eq:0.14.3.3.2} again implies that $\codim(\{x\},X)=\dim(Z)$, and so (since every irreducible component of $X$ contains a closed point, by \sref[0\textsubscript{I}]{0.2.1.3}), (d) implies (b).
\end{proof}
