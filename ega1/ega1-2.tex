...
\begin{env}[2.1.1]
\label{1.2.1.1}
Given a ringed space $(X,\sh{O}_X)$, we say that an open subset $V$ of $X$ is an \emph{affine open} subset if the ringed space $(V,\sh{O}_X|V)$ is an affine scheme \sref{1.1.7.1}.
\end{env}

\begin{definition}[2.1.2]
\label{1.2.1.2}
We define a prescheme to be a ringed space $(X,\sh{O}_X)$ such that every point of $X$ admits an affine open neighborhood.
\end{definition}
...
\begin{proof}
\label{proof-1.2.1.3}
If $V$ is an arbitrary open neighborhood of $x\in X$, then there exists by hypothesis an open neighborhood $W$ of $x$ such that $(W,\sh{O}_X|W)$ is an affine scheme;
...
The ringed space $(D(f),\sh{O}_X|D(f))$ is thus an affine scheme, isomorphic to $A_f$ \sref{1.1.3.6}, whence the proposition.
\end{proof}
...
If $X$ itself is irreducible and $x$ its generic point then we say that $\sh{O}_x$ is the \emph{ring of rational functions on $X$} (cf.~\textsection7).
\end{env}
...
We say that a prescheme $(X,\sh{O}_X)$ is \emph{locally integral} if every $x\in X$ admits an open neighborhood $U$ such that the prescheme induced on $U$ by $(X,\sh{O}_X)$ is integral.
\end{env}
...
it is clear that, by composing $\vphi_\alpha$ and $\vphi_\beta$ with the restriction homomorphisms to $W$, we obtain the same homomorphism $\Gamma(S,\sh{O}_S)\to\Gamma(W,\sh{O}_X|W)$, so, with the equation $(\theta_\alpha^\sharp)_x=(\vphi_\alpha)_x$ for all $x\in V_\alpha$ and all $\alpha$ \sref{1.1.6.1}, the restriction to $W$ of the morphisms $(\psi_\alpha,\theta_\alpha)$ and $(\psi_\beta,\theta_\beta)$ coincide.
...
\label{1.2.2.5}
Under the hypotheses of Proposition~\sref{1.2.2.4}, let $\vphi:A\to\Gamma(X,\sh{O}_X)$ be a ring homomorphism, $f:(X,\sh{O}_X)\to(S,\sh{O}_S)$ the corresponding morphism of preschemes, $\sh{G}$ (resp. $\sh{F}$) an $\sh{O}_X$-module (resp. $\sh{O}_S$-module), and $M=\Gamma(S,\sh{F})$.
...
Indeed, a section of $\sh{O}_X$ over $X$ has a restriction over $X_1$ (resp. $X_2$), identified with an affine open of $X$, that is a polynomial $f(s)$ (resp. $g(t)$), and it follows from the definitions that we should have $g(t)=f(1/t)$, which is not possible if $f=g\in K$.
\end{env}
...
For all preschemes $Y$ and points $y\in Y$, the local scheme $\Spec(\sh{O}_y)$
...
then $\sh{O}_y$ is canonically identified with $B_y$ \sref{1.1.3.4}, and the canonical homomorphism $B\to B_y$ thus corresponds \sref{1.1.6.1} to a morphism of preschemes $\Spec(\sh{O}_y)\to V$.
...
Then $\psi$ is a homeomorphism from $\Spec(\sh{O}_y)$ to the subspace $S_y$ of $Y$ given by the $z$ such that $y\in\overline{\{z\}}$ (\emph{or, equivalently, the generalizations of $y$ \sref[0]{0.2.1.2}};
...
\emph{We see \sref{1.2.1.5} that there is a bijective correspondence between $\Spec(\sh{O}_y)$ and the set of closed irreducible subsets of $Y$ containing $y$.}
...
So it suffices to consider the case where $(Y,\sh{O}_Y)$ is an affine scheme of ring $B$, and then we have that $u=({}^a\vphi,\tilde{\vphi})$, where $\vphi\in\Hom(B,A)$ \sref{1.1.7.3}.
...
Conversely, to each local homomorphism $\sh{O}_y\to A$ there is a unique corresponding morphism $(\psi,\theta):X\to\Spec(\sh{O}_y)$ such that $\psi(a)=y$ \sref{1.1.7.3}, and, by composing with the canonical morphism $\Spec(\sh{O}_y)\to Y$, we obtain a morphism $X\to Y$, which proves the proposition.
...
For $\mathfrak{a}_y=\mathfrak{m}_y$, this says that $\sh{O}_y/\mathfrak{a}_y=\kres(y)$, and so Proposition~\sref{1.2.4.4} says that:
...
This establishes a canonical bijective correspondence between the set of morphisms $(X,\sh{O}_X)\to (Y,\sh{O}_Y)$ and the set of monomorphisms $\kres(y)\to K$ (for $y\in Y$).
\end{corollary}

\begin{corollary}[2.4.7]
\label{1.2.4.7}
For all $y\in Y$, every canonical morphism $\Spec(\sh{O}_y/\mathfrak{a}_y)\to Y$ is a monomorphism of ringed spaces.
\end{corollary}
...
Since every affine open subset containing $a$ is necessarily equal to the whole of $X$, every \emph{invertible} $\sh{O}_X$-module \sref[0]{0.5.4.1} is necessarily \emph{isomorphic to $\sh{O}_X$} (or, as we say, again, \emph{trivial}).
...
It follows from \sref{1.2.2.4} that the data of a prescheme over a ring $A$ is equivalent to the data of a prescheme $(X,\sh{O}_X)$ whose structure sheaf $\sh{O}_X$ is a sheaf of \emph{$A$-algebras}.
