...
so there exists some $f\in A$ such that $D(f)$ is an open neighborhood of $x$ contained inside $V\cap W$ \sref{I.1.1.10}[i].
The ringed space $(D(f),\sh{O}_X|D(f))$ is thus an affine scheme, isomorphic to $A_f$ \sref{I.1.3.6}, whence the proposition.
\end{proof}
...
\begin{proof}
If $x$ and $y$ are two distinct points of a prescheme $X$, then it is clear that there exists an open neighborhood of one of these points that does not contain the other if $x$ and $y$ are not in the same affine open subset; and if they are in the same affine open subset, this is a result of \sref{I.1.1.8}.
\end{proof}
...
The uniqueness of the generic point of $X$ is a result of Proposition~\sref{I.2.1.4} and of \sref[0]{0.2.1.3}.
\end{proof}
...
Furthermore, for each pair of indices $(\alpha,\beta)$, each point of $V_\alpha\cap V_\beta$ admits an affine open neighborhood $W$ contained inside $V_\alpha\cap V_\beta$ \sref{I.2.1.3};
it is clear that, by composing $\vphi_\alpha$ and $\vphi_\beta$ with the restriction homomorphisms to $W$, we obtain the same homomorphism $\Gamma(S,\sh{O}_S)\to\Gamma(W,\sh{O}_X|W)$, so, with the equation $(\theta_\alpha^\sharp)_x=(\vphi_\alpha)_x$ for all $x\in V_\alpha$ and all $\alpha$ \sref{I.1.6.1}, the restriction to $W$ of the morphisms $(\psi_\alpha,\theta_\alpha)$ and $(\psi_\beta,\theta_\beta)$ coincide.
...
\label{I.2.2.5}
Under the hypotheses of Proposition~\sref{I.2.2.4}, let $\vphi:A\to\Gamma(X,\sh{O}_X)$ be a ring homomorphism, $f:(X,\sh{O}_X)\to(S,\sh{O}_S)$ the corresponding morphism of preschemes, $\sh{G}$ (resp. $\sh{F}$) an $\sh{O}_X$-module (resp. $\sh{O}_S$-module), and $M=\Gamma(S,\sh{F})$.
...
\label{I.2.3.1}
\oldpage[I]{101}
It follows from Definition~\sref{I.2.1.2} that every ringed space obtained by \emph{gluing} preschemes \sref[0]{0.4.1.7} is again a prescheme.
...
We later show that the prescheme $X$ obtained in this manner is a particular case of a general method of construction \sref[II]{II.2.4.3}.
...
Then $\psi$ is a homeomorphism from $\Spec(\sh{O}_y)$ to the subspace $S_y$ of $Y$ given by the $z$ such that $y\in\overline{\{z\}}$ \emph{(or, equivalently, the \emph{generalizations} of $y$ \sref[0]{0.2.1.2})};
...
but then this proposition follows from \sref{I.1.6.2}.
\end{proof}

\emph{We see \sref{I.2.1.5} that there is a bijective correspondence between $\Spec(\sh{O}_y)$ and the set of closed irreducible subsets of $Y$ containing $y$.}
...
For $\mathfrak{a}_y=\mathfrak{m}_y$, this says that $\sh{O}_y/\mathfrak{a}_y=\kres(y)$, and so Proposition~\sref{I.2.4.4} says that:
