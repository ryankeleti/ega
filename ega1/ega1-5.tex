...
Additionally, since $\sh{I}^2=0$, $\sh{I}$ is a quasi-coherent $(\OO_X/\sh{I})$-module, so we have $\sh{I}\cong\wt{\mathfrak{K}}$ and $\mathfrak{K}_x=\sh{I}_x$ for all $x\in X_0$ \sref{1.1.4.1}.
...
Furthermore, the equation $\mathfrak{K}_x=\sh{I}_x$ shows that the restriction of $\theta^\sharp:\psi^*(\OO_{X'})\to\OO_X$ is an \emph{isomorphism} from $\psi^*(\wt{\mathfrak{K}})$ to $\sh{I}$;
additionally, by passing to the quotients, $\theta^\sharp$ gives an \emph{isomorphism} $\psi^*(\OO_X/\wt{\mathfrak{K}})\to\OO_X/\sh{I}$, because $f_0$ is an isomorphism;
...
\begin{defn}[5.4.1]
\label{1.5.4.1}
We say that a morphism of preschemes $f:X\to Y$ is separated if the diagonal morphism $X\to X\times_Y X$ is a \emph{closed} immersion; we then also say that $X$ is a \emph{separated prescheme over $Y$}, or a \emph{$Y$-scheme}.
...
For a morphism $f:X\to Y$ to be separated, it is necessary and sufficient for $U_\alpha\cap U_\beta$ to be, for every pair of indices $(\alpha,\beta)$, an affine open subset, \emph{and} for the ring $\Gamma(U_\alpha\cap U_\beta,\OO_X)$ to be generated by the union of the canonical images of the rings $\Gamma(U_\alpha,\OO_X)$ and $\Gamma(U_\beta,\OO_X)$.
\end{prop}
