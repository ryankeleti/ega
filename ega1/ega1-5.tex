...
Additionally, since $\sh{I}^2=0$, $\sh{I}$ is a quasi-coherent $(\sh{O}_X/\sh{I})$-module, so we have $\sh{I}\cong\widetilde{\mathfrak{K}}$ and $\mathfrak{K}_x=\sh{I}_x$ for all $x\in X_0$ \sref{1.1.4.1}.
...
If $A$ is the ring of $Y$, then we have $\sh{F}=\widetilde{M}$ and $\sh{G}=\widetilde{N}$, where $M$ and $N$ are $A$-modules, and, by hypothesis, $N$ is an extension of the $A$-module $A$ by the $A$-module $M$ \sref{1.1.3.11}.
...
\begin{enumerate}
  ...
  \item[{\rm(vi)}] For a morphism $f$ to be separated, it is necessary and sufficient for $f_\red$ \sref{1.5.1.5} to be separated.
\end{enumerate}
\end{proposition}
...
\oldpage[I]{140}
  ...
  \item[(vi)] \emph{If a morphism $f:X\to Y$ has property~\textbf{P}, then so too does $f_\red$ \sref{1.5.1.5}.}
\end{enumerate}
