...
We know that $\wt{\nilrad}$ is a quasi-coherent $\sh{O}_X$-module \sref{1.1.4.1}, and that its stalk at a point $x\in X$ is the ideal $\nilrad_x$ of the ring of fractions $B_x$;
...
We can restrict to the case where $X$ is affine, say $X=\Spec(A)$ and $\sh{I}=\wt{\mathfrak{I}}$, where $\mathfrak{I}$ is an ideal of $A$;
...
Since every ring of fractions $\neq\{0\}$ of an integral ring is integral, it follows from \sref{1.5.1.4} that, for every \emph{locally integral} prescheme $X$, $\sh{O}_x$ is an \emph{integral} ring for every $x\in X$.
...
Finally, if $f=(\psi,\theta)$ is an immersion, a closed immersion, or a local immersion (resp. an open immersion, or a local isomorphism), then the proposition follows from the fact that, if $\theta_x^\sharp$ is surjective (resp. bijective), then so too is the homomorphism obtained by passing to the quotients by the nilradicals $\sh{O}_{\psi(x)}$ and $\sh{O}_x$ (\sref{1.5.1.2} and \sref{1.4.2.2}) (cf. \sref{1.5.5.12}).
\end{proof}
...
Additionally, since $\sh{I}^2=0$, $\sh{I}$ is a quasi-coherent $(\sh{O}_X/\sh{I})$-module, so we have $\sh{I}\cong\wt{\mathfrak{K}}$ and $\mathfrak{K}_x=\sh{I}_x$ for all $x\in X_0$ \sref{1.1.4.1}.
With this in mind, let $X'=\Spec(A)$, and consider the morphism $f=(\psi,\theta):X\to X'$ of preschemes that corresponds to the identity map $A\to\Gamma(X,\sh{O}_X)$ \sref{1.2.2.4}.
...
Furthermore, the equation $\mathfrak{K}_x=\sh{I}_x$ shows that the restriction of $\theta^\sharp:\psi^*(\sh{O}_{X'})\to\sh{O}_X$ is an \emph{isomorphism} from $\psi^*(\wt{\mathfrak{K}})$ to $\sh{I}$;
additionally, by passing to the quotients, $\theta^\sharp$ gives an \emph{isomorphism} $\psi^*(\sh{O}_X/\wt{\mathfrak{K}})\to\sh{O}_X/\sh{I}$, because $f_0$ is an isomorphism;
...
$\mathfrak{j}(Y)$ is the largest ideal $\mathfrak{a}\subset A$ such that $V(\mathfrak{a})=Y$, and it is equal to its radical \sref{1.1.1.4}[i], so $A/\mathfrak{j}(Y)$ is a reduced ring.
...
\begin{enumerate}[label=\emph{(\roman*)}]
  ...
  \item If $f:X\to X'$ and $g:Y\to Y'$ are separated $S$-morphisms, then $f\times_S g$ is separated.
...
\label{eq:1.5.5.1.1}
  ...
  \tag{5.5.1.1}
\]
commutes (where $j$ denotes the canonical immersion \sref{1.5.3.10}), as can be immediately verified.
...
Finally, to prove (vi), recall that the preschemes $X_\red\times_{Y_\red}X_\red$ and $X_\red\times_Y X_\red$ are canonically identified with one another \sref{1.5.1.7};
...
\]
commutes \sref{1.5.3.15}, and the proposition follows from the fact that the vertical arrows are homeomorphisms of the underlying spaces \sref{1.4.3.1}.
\end{proof}
...
\begin{proof}
\label{proof-1.5.5.2}
This follows from \sref{1.5.5.1}[i and ii].
\end{proof}
...
\begin{proof}
\label{proof-1.5.5.4}
The necessity follows from \sref{1.5.5.1}[i, ii, and v].
...
if $p_1$ and $p_2$ are the projections of $X\times_Y X$, then the subspace $\Delta_{X_k}(X_k)$ can be identified with the subspace $\Delta_X(X)\cap p_1^{-1}(X_k)$ of the underlying space of $X\times_Y X$ \sref{1.5.3.16};
...
Proposition~\sref{1.5.5.4} then, in this case, leads to the idea of separation in the case of \emph{integral} preschemes \sref{1.2.1.7}.
...
\begin{proof}
\label{proof-1.5.5.5}
If we set $X_\lambda=f^{-1}(Y_\lambda)$, everything reduces, by taking \sref{1.4.2.4}[b] and the identification of the products $X_\lambda\times_Y X_\lambda$ and $X_\lambda\times_{Y_\lambda}X_\lambda$ into account, to proving that the $X_\lambda\times_Y X_\lambda$ form a cover of $X\times_Y X$.
But if we set $Y_{\lambda\mu}=Y_\lambda\cap Y_\mu$ and $X_{\lambda\mu}=X_\lambda\cap X_\mu=f^{-1}(Y_{\lambda\mu})$, then $X_\lambda\times_Y X_\mu$ can be identified with the product $X_{\lambda\mu}\times_{Y_{\lambda\mu}}X_{\lambda\mu}$ \sref{1.3.2.6.4}, and so also with $X_{\lambda\mu}\times_Y X_{\lambda\mu}$ \sref{1.3.2.5}, and finally with an open subset of $X_\lambda\times_Y X_\lambda$, which proves our claim \sref{1.3.2.7}.
\end{proof}

Proposition~\sref{1.5.5.4} allows us, by taking an affine open cover of $Y$, to restrict our study of separated morphisms to just those that take values in affine schemes.
...
Since $U_\alpha\times_Y U_\beta$ is an affine scheme whose ring is canonically isomorphic to $\Gamma(U_\alpha,\sh{O}_X)\otimes_{\Gamma(Y,\sh{O}_Y)}\Gamma(U_\beta,\sh{O}_X)$ \sref{1.3.2.2}, we see that $U_\alpha\cap U_\beta$ must be an affine scheme, and that the map $h_\alpha\otimes h_\beta\mapsto h_\alpha h_\beta$ from the ring $A(U_\alpha\times_Y U_\beta)$ to $\Gamma(U_\alpha\cap U_\beta,\sh{O}_X)$ must be surjective \sref{1.4.2.3}, which finishes the proof.
\end{proof}
...
\begin{proof}
\label{proof-1.5.5.8}
Indeed, we see that the criteria of \sref{1.5.5.6} do not depend on $f$.
\end{proof}
...
\label{1.5.5.11}
The prescheme from Example~\sref{1.2.3.2} (``the projective line over a field $K$'') is \emph{separated}, because, for the cover $(X_1,X_2)$ of $X$ by affine open subsets, $X_1\cap X_2=U_{12}$ is affine, and $\Gamma(U_{12},\sh{O}_X)$, the ring of rational fractions of the form $f(s)/s^m$ with $f\in K[s]$, is generated by $K[s]$ and $1/s$, so the conditions of \sref{1.5.5.6} are satisfied.
...
Indeed, if $z$ is a section of $\sh{O}_Y$ over $U$, there exist two integers $m,n\geq0$ such that $s^mz$ and $t^nz$ are the restrictions of polynomials in $s$ and $t$ to $U$ \sref{1.1.4.1}, which is clearly possible only if the section $z$ extends to a section over the whole of $Y$, identified with a polynomial in $s$ and $t$.
...
we then have an example where the conditions of \sref{1.5.5.6} are not satisfied (the integral prescheme thus obtained is called ``the affine plane over $K$ with the point $0$ doubled'').
\end{example}
...
\]
where the vertical arrows are the closed immersions \sref{1.5.1.5}, and thus have property~\textbf{P}, by (i).
