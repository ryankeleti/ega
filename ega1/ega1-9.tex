...
denoted by $\sh{F}\otimes_{\OO_S}\sh{G}$
(\emph{or} $\sh{F}\otimes_S\sh{G}$) to be the tensor product
$p^*(\sh{F})\otimes_{\OO_{X\times_S Y}}q^*(\sh{G})$ over the prescheme $X\times_S Y$.
\end{defn}
...
\oldpage[I]{173}
  \item Again, $X$ is a finite union of affine opens $U_i$ ($1\leq i\leq r$)
    ...
    $s\otimes f^{\otimes(n+m)}$ over $X_f$.\qedhere
\end{enumerate}
\end{proof}
...
of $\OO_X|\sh{J}$. Then there exists an integer $n>0$ such that $\sh{J}^n\sh{F}=0$.
\end{prop}
...
quasi-coherent $\OO_X$-module, and $\sh{G}$ a quasi-coherent $(\OO_X|U)$-submodule of $\sh{F}|U$ \emph{of finite type}. Then there exists a quasi-coherent $\OO_X$-submodule $\sh{G}'$ of
...
\sref{1.9.4.2}, so the inductive limit of its quasi-coherent $\OO_X$-submodules
...
and for every quasi-coherent $(\OO_X|U)$-submodule $\sh{G}$ of $\sh{F}|U$ of finite type, it
follows
\oldpage[I]{175}
...
$\sh{G}'_\lambda$ to be the unique $(\OO_X|U_\lambda)$-submodule of $\sh{F}|U_\lambda$ such that
...
\]
and then to take $\sh{G}'_\lambda$ to be the $(\OO_X|U_\lambda)$-submodule of
...
$\OO_Y$-module (which will be the case if $f$ is quasi-compact \emph{and}, in addition, either $f$ is separated or $X$ is locally Noetherian \sref{1.9.2.2}). Then there exists a smaller
...
\]
since it is the image under $\theta$ of the unit element of $\OO_y$, the latter
...
\begin{proof}
\label{proof-1.9.5.8}
Let $X'=f^{-1}(V)$; since the direct image of $\OO_{X'}$ by $f_V$ is exactly the restriction of $f_*(\OO_X)$ to $V$, it is clear that the kernel $\sh{J}'$ of the homomorphism $\OO_V\to(f_V)_*(\OO_{X'})$ is the restriction of $\sh{J}$ to $V$, from which the proposition quickly follows.
\end{proof}
...
If we take into account that the sections of $\OO_{\overline{Y}}$ over $\overline{Y}$ canonically correspond to the $\overline{Y}$-sections of $\overline{Y}\otimes_Z Z[T]$ \sref{1.3.3.15} and that the latter is separated over $\overline{Y}$, then the corollary appears as a specific case of \sref{1.9.5.6}.
\end{proof}
...
\begin{prop}[9.6.1]
\label{1.9.6.1}
Let $X$ be a prescheme, and $\sh{B}$ a quasi-coherent $\OO_X$-algebra \sref[0]{0.5.1.3}.
...
If this is the case, and if $\sh{G}$ is a $\sh{B}$-submodule or a quotient module of $\sh{F}$, then in order for $\sh{G}$ to be a coherent $\sh{B}$-module, it is necessary and sufficient that it be a quasi-coherent $\OO_X$-module.
\end{cor}
