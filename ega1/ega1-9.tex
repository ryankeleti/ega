...
\emph{quasi-coherent} $\sh{O}_Z$-module by virtue of \sref{1.9.1.1} and
...
$\sh{O}_{X\times_S Y}$, we reduce, by Proposition~\sref{1.9.1.12} and
...
\begin{proof}
\label{proof-1.9.2.2}
In case~(a), the $X_{\alpha i}\cap X_{\alpha j}$ are affine \sref{1.5.5.6}.
Case~(b) is a particular example of case~(a) \sref{1.6.6.3}. Finally, in case~(c), we can reduce to the case where $Y$ is affine and the underlying space of $X$ is Noetherian; then $X$ admits a finite cover of affine opens $(X_i)$, and the $X_i\cap X_j$, being quasi-compact, are finite unions of affine opens \sref{1.2.1.3}.
\end{proof}
...
where $\mathfrak{J}=\Gamma(X,\sh{J})$ is an ideal of $A$ (\sref{1.1.4.1} and
\sref{1.1.5.1}). Since $A$ is Noetherian, $\mathfrak{J}$ admits a finite system of
...
$\sh{J}^n\sh{F}=\widetilde{\mathfrak{J}^n M}$ \sref{1.1.3.13} is zero.
\end{proof}
...
\sref{1.9.4.2}, so the inductive limit of its quasi-coherent $\sh{O}_X$-submodules
...
\label{1.9.4.5}
Under the hypotheses of Corollary~\sref{1.9.4.3}, for every quasi-coherent $(\sh{O}_X|U)$-module
...
\label{1.9.4.8}
Under the hypotheses of \sref{1.9.4.7}, for every quasi-coherent $(\sh{O}_X|U)$-module
$\sh{G}$ of finite type, there exists a quasi-coherent $\sh{O}_X$-module $\sh{G}'$ of finite
type such that $\sh{G}'|U=\sh{G}$.
\end{corollary}
...
\label{1.9.4.10}
Under the hypotheses of \sref{1.9.4.9}, if a quasi-coherent $\sh{O}_X$-module $\sh{F}$ is
...
$\sh{O}_Y$-module (which will be the case if $f$ is quasi-compact \emph{and}, in addition, either $f$ is separated or $X$ is locally Noetherian \sref{1.9.2.2}). Then there exists a smaller
...
$f_0=(\psi_0,\theta_0)$ is a morphism of preschemes $X\to Y'$ \sref{1.2.2.1} such that
...
the notation of \sref{1.9.5.2}) $\sh{J}_y=\sh{O}_y$ for
...
$f$ factors through the injection $g^{-1}(Z_1)\to Y$ \sref{1.4.4.1}. By
...
From this, we conclude \sref{1.4.4.1} that the $f$ factors through the canonical injection $Y'\to Y$, so $Y'=Y$ by hypothesis; it then follows \sref{1.4.4.1} that $h$ factors as $\Delta_Z\circ v$, where $v$ is a morphism $Y\to Z$, which implies that $g_1=g_2=v$.
\end{proof}

\begin{remark}[9.5.7]
\label{1.9.5.7}
If $X$ and $Y$ are $S$-schemes, Proposition~\sref{1.9.5.6} implies that, when
\oldpage[I]{178}
...
\begin{enumerate}
  ...
  \item[{\rm(ii)}] If the hypotheses of Proposition~\sref{1.9.5.1} are satisfied and $X$ is irreducible (resp. integral), then so too is $Y'$.
\end{enumerate}
\end{proposition}
...
Since $X$ is reduced, $g$ factors as $X\xrightarrow{h} {Y'}_\red\xrightarrow{j'}Y'$, where $j'$ is the canonical injection \sref{1.5.2.2}, and it then follows from the definition of $Y'$ that $Y'_\red=Y'$.
...
It suffices to apply Proposition~\sref{1.9.5.1} to the injection $j$, which is separated \sref{1.5.5.1} and quasi-compact by hypothesis; \sref{1.9.5.1} thus proves the existence of $\overline{Y}$, and \sref{1.9.5.4} shows that its underlying space is the closure of $Y$ in $X$; since $Y$ is locally closed in $X$, it is open in $\overline{Y}$, and the last claim comes from \sref{1.9.5.8} applied to an open subset $V$ of $X$ such that $Y$ is closed in $V$.
\end{proof}

With the above notation, if the injection $V\to X$ is quasi-compact, and if $\sh{J}$ is the quasi-coherent sheaf of ideals of $\sh{O}_X|V$ defining the closed subprescheme $Y$ of $V$, it follows from Proposition~\sref{1.9.5.1} that the quasi-coherent sheaf of ideals of $\sh{O}_X$ defining $\overline{Y}$ is the canonical extension \sref{1.9.4.1} $\overline{\sh{J}}$ of $\sh{J}$, because it is clearly the largest quasi-coherent subsheaf of ideals of $\sh{O}_X$ inducing $\sh{J}$ on $V$.

\begin{corollary}[9.5.11]
\label{1.9.5.11}
Under
\oldpage[I]{179}
the hypotheses of Proposition~\sref{1.9.5.10}, every section of $\sh{O}_{\overline{Y}}$ over an open $V$ of $\overline{Y}$ that is zero on $V\cap Y$ is zero.
\end{corollary}

\begin{proof}
\label{proof-1.9.5.11}
By Proposition~\sref{1.9.5.8}, we can reduce to the case where $V=\overline{Y}$.
If we take into account that the sections of $\sh{O}_{\overline{Y}}$ over $\overline{Y}$ canonically correspond to the $\overline{Y}$-sections of $\overline{Y}\otimes_Z Z[T]$ \sref{1.3.3.15} and that the latter is separated over $\overline{Y}$, then the corollary appears as a specific case of \sref{1.9.5.6}.
\end{proof}
...
\begin{proof}
\label{proof-1.9.6.1}
Since the question is local, we can assume $X$ to be affine, given by some ring $A$, and thus $\sh{B}=\widetilde{B}$, where $B$ is an $A$-algebra \sref{1.1.4.3}.
If $\sh{F}$ is quasi-coherent on the ringed space $(X,\sh{B})$ then we can also assume that $\sh{F}$ is the cokernel of a $\sh{B}$-homomorphism $\sh{B}^{(I)}\to\sh{B}^{(J)}$; since this homomorphism is also an $\sh{O}_X$-homomorphism of $\sh{O}_X$-modules, and $\sh{B}^{(I)}$ and $\sh{B}^{(J)}$ are quasi-coherent $\sh{O}_X$-modules \sref{1.1.3.9}[ii], $\sh{F}$ is also a quasi-coherent $\sh{O}_X$-module \sref{1.1.3.9}[i].

Conversely, if $\sh{F}$ is a quasi-coherent $\sh{O}_X$-module, then $\sh{F}=\widetilde{M}$, where $M$ is a $B$-module \sref{1.1.4.3}; $M$ is isomorphic to the cokernel of a $B$-homomorphism $B^{(I)}\to B^{(J)}$, so $\sh{F}$ is a $\sh{B}$-module isomorphic to the cokernel of the corresponding homomorphism $\sh{B}^{(I)}\to\sh{B}^{(J)}$ \sref{1.1.3.13}, which finishes the proof.
\end{proof}

In particular, if $\sh{F}$ and $\sh{G}$ are two quasi-coherent $\sh{B}$-modules, then $\sh{F}\otimes_{\sh{B}}\sh{G}$ is a quasi-coherent $\sh{B}$-module; similarly for $\shHom(\sh{F},\sh{G})$ whenever we further suppose that $\sh{F}$ admits a finite presentation \sref{1.1.3.13}.
...
\oldpage[I]{180}
of finite type; but it is isomorphic (as a $\sh{B}$-module) to $\widetilde{N}$, where $N$ is the kernel of the corresponding homomorphism of $B$-modules $B^m\to B$ \sref{1.1.3.13}.
...
\label{1.9.6.4}
Under the hypotheses of \sref{1.9.6.3}, for a $\sh{B}$-module $\sh{F}$ to be coherent, it is necessary and sufficient that it be a quasi-coherent $\sh{O}_X$-module and a $\sh{B}$-module of finite type.
...
\begin{proof}
\label{proof-1.9.6.4}
Taking \sref{1.9.6.1} into account, the conditions on $\sh{F}$ are clearly necessary; we will show that they are sufficient.
...
\begin{proof}
\label{proof-1.9.6.5}
In fact, by hypothesis, there exists a finite cover $(U_i)$ of $X$ consisting of affine opens such that $\Gamma(U_i,\sh{B})=B_i$ is an algebra of finite type over $\Gamma(U_i,\sh{O}_X)=A_i$; let $E_i$ be a $A_i$-submodule of $B_i$ of finite type that generates the $A_i$-algebra $B_i$; thanks to \sref{1.9.4.7}, there exists a $\sh{O}_X$-submodule $\sh{E}_i$ of $\sh{B}$, quasi-coherent and of finite type, such that $\sh{E}_i|U_i=\widetilde{E_i}$.
...
\begin{proof}
\label{proof-1.9.6.6}
In fact, it follows from \sref{1.9.4.9} that $\sh{B}$ is the inductive limit (as an $\sh{O}_X$-module) of its quasi-coherent $\sh{O}_X$-submodules of finite type; the latter generating quasi-coherent \emph{$\sh{O}_X$-subalgebras} of $\sh{B}$ of finite type \sref{1.1.3.14}, and so $\sh{B}$ is \emph{a fortiori} their inductive limit.
\end{proof}
