...
Indeed, if $X=\Spec(A)$, then we have $\OO_X=\wt{A}$ and $\sh{I}=\wt{\mathfrak{I}}$, where $\mathfrak{I}$ is an ideal of $A$ \sref{1.1.4.1};
...
in addition, if $\vphi$ is the canonical homomorphism $A\to B=A/\mathfrak{I}$, then the direct image ${}^a\vphi_*(\wt{B})$ is canonically identified with the sheaf $\wt{A}/\wt{\mathfrak{I}}=\OO_X/\sh{I}$ (Proposition~\sref{1.1.6.3} and Corollary~\sref{1.1.3.9}), which finishes the proof.
\end{proof}
...
\oldpage[I]{123}
    ...
    As a result, there exists a unique isomorphism $\omega$ from a sheaf quotient $\wt{A}/\wt{\mathfrak{J}}$ (with $\mathfrak{J}$ being an ideal of $A$) to $f_*(\OO_Y)$ which, when composed with the canonical homomorphism $\wt{A}\to\wt{A}/\wt{\mathfrak{J}}$, gives $\theta$ \sref{1.1.3.8};
    if $\OO_Z$ denotes the restriction of $\wt{A}/\wt{\mathfrak{J}}$ to $Z$, then $(Z,\OO_Z)$ is a subprescheme of $(X,\OO_X)$, and $f$ factors through the canonical injection of this subprescheme into $X$ and the isomorphism $(\psi_0,\omega_0)$, where $\psi_0$ is $\psi$ considered as a map from $Y$ to $Z$, and $\omega_0$ the restriction of $\omega$ to $\OO_Z$.
