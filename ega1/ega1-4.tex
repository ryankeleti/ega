...
\begin{enumerate}[label=(\alph*)]
  ...
  \item The conditions are evidently necessary---we prove that they are sufficient.
    ...
    $\psi(V)$ is then open in $Z$, so \unsure{trace} over $Z$ of an open subset $U$ of $X$, and the restriction of $U$ to $\psi_*(\OO_Y)$ is identical to the restriction of $U$ to the direct image
\oldpage[I]{123}
...
Indeed, let $(S_\lambda)$ be a cover of $S$ by affine open sets; if $\vphi$ and $\psi$ are the structure morphisms of $X$ and $Y$, then let $X_\lambda=\vphi^{-1}(S_\lambda)$ and $Y_\lambda=\psi^{-1}(S_\lambda)$.
