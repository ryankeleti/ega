...
With these conditions, the completion $Z_{/Z'}$ indentifies with the product of formal $S_{/S'}$-preschemes $(X_{/X'})\times_{S_{/S'}}(Y_{/Y'})$, the structure morphisms indentify with $\wh{g}$ and $\wh{h}$, and the projections with $\wh{p}$ and $\wh{q}$.
\end{prop}
...
\label{1.10.9.10}
\oldpage[I]{201}
If $S$ is the sum $X_1\amalg X_2$ (3.1), $X'$ the union $X_1'\cup X_2'$, where $X_i'$ is a closed subset of $X_i$ ($i=1,2$), then we have $X_{/X'}={X_1}_{/X_1'}\amalg{X_2}_{/X_2'}$.
\end{rmk}
...
In addition, the Definitions \sref{1.10.1.2} and \sref{1.10.8.4} show that the \emp{formal affine scheme $\fk{X}$} is identical the completion $X_{/\fk{X}}$ of the affine scheme $X$ along the closed subset $\fk{X}$ of its underling space.
...
Since the sheaf of ideals of $\OO_{S_n}$ that defines the subprescheme $S_m$ of $S_n$ is nilpotent, so too, by \sref{1.4.4.5}, is the sheaf of ideals of $\OO_{X_n}$ that defines the subprescheme $X_m$ of $X_n$, so the conditions of \sref{1.10.6.3} are satisfied.
...
\[
    \Hom_\fk{S}(\fk{X},\fk{Y}) \xrightarrow{\sim} \varprojlim_n\Hom_{S_n}(X_n,Y_n)
\]
...
\oldpage[I]{208}
    \item Every point of $\fk{Y}$ has a Noetherian formal affine open neighbourhood $V$ that has the following property:
        ...
        (\textbf{Q}) $f^{-1}(V)$ is a finite union of Noetherian formal affine open subsets $U_i$ such that the Noetherian adic ring $\Gamma(U_i,\OO_\fk{X})$ is topologically isomorphic to the quotient of a formal series algebra, restricted \sref[0]{0.7.5.1} to $\Gamma(V,\OO_\fk{Y})$, by an ideal (necessarily closed).
\end{enumerate}
\end{prop}

\begin{proof}
\label{proof-1.10.13.1}
It is immediate that \emph{(a)} implies \emph{(b)} by \sref{1.10.12.3}.
To show that \emph{(b)} implies \emph{(c)}, we can, since the question is local on $\fk{Y}$, assume that $\fk{Y}=\Spf(B)$, where $B$ is Noetherian and adic;
...
By \sref{1.5.1.9}, $U_i$ is also an affine open subset in each of the Noetherian preschemes $X_n$, and if $A_{in}$ is the ring of the affine scheme induced by $X_n$ on $U_i$, then hypothesis \emph{(b)} implies that, for $m\leqslant n$, $A_{im}$ is isomorphic to $A_{in}/\fk{K}^{m+1}A_{in}$.
...
To show that \emph{(c)} implies \emph{(a)}, we can restrict to the case where $\fk{X}=\Spf(A)$ is also affine, with $A$ a Noetherian adic ring that is isomorphic to a quotient of a formal series algebra, restricted to $B$, by a closed ideal.
Then \sref[0]{0.7.5.5} $A/\fk{K}A$ is an algebra of finite type over $B/\fk{K}$, and $\fk{K}A=\fk{J}$ is an ideal of definition of $A$, and so, by \sref{1.10.10.9}, the conditions of \emph{(a)} are satisfied.
\end{proof}

We note that, if the conditions of prop.~\sref{1.10.13.1} are satisfied, then property~\emph{(a)} holds true for \emph{any} ideal of definition $\sh{K}$ of $\fk{Y}$ (by \emph{(c)}), and so, in property~\emph{(b)}, \emph{all} the $f_n$ are morphisms of finite type.

\begin{cor}[10.13.2]
\label{1.10.13.2}
If the conditions of \sref{1.10.13.1} are satisfied, then every Noetherian formal affine open subset $V$ of $\fk{Y}$ has property~(\textbf{Q}), and if $\fk{Y}$ is Noetherian, then so too is $\fk{X}$.
\end{cor}
...
\begin{defn}[10.13.3]
\label{1.10.13.3}
When the equivalent properties \emph{(a)}, \emph{(b)}, and \emph{(c)} of \sref{1.10.13.1} are satisfied, we say that the morphism $f$ is of finite type, or that $\fk{X}$ is a formal $\fk{Y}$-prescheme of finite type, or a formal prescheme of finite type over $\fk{Y}$.
\end{defn}
...
\oldpage[I]{209}
    \item Let $\fk{X}$, $\fk{G}$, and $\fk{G}'$ be locally Noetherian (\emph{resp.} Noetherian) formal preschemes, and $f:\fk{X}\to\fk{G}$ and $\fk{X}\to\fk{G}'$ morphisms.
        If $f$ is of finite type, then $\fk{X}\times_\fk{G}\fk{G}'$ is locally Noetherian (\emph{resp.} Noetherian) and of finite type over $\fk{G}'$.
    \item Let $\fk{G}$ be a locally Noetherian formal prescheme, and $\fk{X}'$ and $\fk{Y}'$ formal $\fk{G}$-preschemes such that $\fk{X}'\times_\fk{G}\fk{Y}'$ is locally Noetherian.
    If $\fk{X}$ and $\fk{Y}$ are locally Noetherian formal $\fk{G}$-preschemes, and $f:\fk{X}\to\fk{X}'$ and $g:\fk{Y}\to\fk{Y}'$ are $\fk{G}$-morphisms of finite type, then $\fk{X}\times_\fk{G}\fk{Y}$ is locally Noetherian, and $f\times_\fk{G}g$ is a $\fk{G}$-morphism of finite type.
\end{enumerate}
\end{prop}
...
Under the conditions of (ii), $\fk{G}$ (resp. $\fk{X}$, $\fk{G}'$) is the inductive limit of a sequence $(S_n)$ (resp. $(X_n)$, $(S'_n)$) of locally Noetherian preschemes, and we can assume \sref{1.10.13.1} that $X_m=X_n\times_{S_n}S_m$ for $m\leqslant n$.
The formal prescheme $\fk{X}\times_\fk{G}\fk{G}'$ is then the inductive limit of the preschemes $X_n\times_{S_n}S'_n$ \sref{1.10.7.4}, and we have
\[
...
We thus conclude \sref{1.10.12.3.1}, first of all, that $\fk{X}\times_\fk{G}\fk{G}'$ is locally Noetherian;
then, since $X_0\times_{S_0}S'_0$ is of finite type over $S'_0$ \sref{1.6.3.8}, it follows from \hyperref[1.10.12.3]{(10.12.3.1)} and \sref{1.10.13.1} that $\fk{X}\times_\fk{G}\fk{G}'$ is of finite type over $\fk{G}'$, which proves (ii) (the claim about Noetherian preschemes being an immediate consequence of \sref{1.6.3.8}).
\end{proof}
...
\oldpage[I]{211}
    \item Let $\fk{X}$, $\fk{Y}$, and $\fk{G}$ be locally Noetherian formal preschemes, $f:\fk{X}\to\fk{G}$ a closed immersion, and $g:\fk{Y}\to\fk{G}$ a morphism.
        Then the morphism $\fk{X}\times_\fk{G}\fk{Y}\to\fk{Y}$ is a closed immersion.
    \item Let $\fk{G}$ be a locally Noetherian formal prescheme, and $\fk{X}'$ and $\fk{Y}'$ locally Noetherian formal $\fk{G}$-preschemes such that $\fk{X}'\times_\fk{G}\fk{Y}'$ is locally Noetherian.
    If $\fk{X}$ and $\fk{Y}$ are locally Noetherian $\fk{G}$-preschemes, and $f:\fk{X}\to\fk{X}'$ and $g:\fk{Y}\to\fk{Y}'$ are $\fk{G}$-morphisms that are closed immersions, then $f\times_\fk{G}g$ is a closed immersion.
\end{enumerate}
\end{prop}
...
To prove (ii), it is immediate that we can restrict to the case where $fk{G}=\Spf(A)$, $\fk{X}=\Spf(B)$, and $\fk{Y}=\Spf(C)$, with $A$ a Noetherian $\fk{J}$-adic ring, $B=A/\fk{a}$ (where $\fk{a}$ is an ideal of $A$), and $C$ a Noetherian topological adic $A$-algebra.
...
\begin{cor}[10.14.6]
\label{1.10.14.6}
Under the hypotheses of \sref{1.10.14.5}[ii], let $p:\fk{X}\times_\fk{G}\fk{Y}\to\fk{X}$ and $q:\fk{X}\times_\fk{G}\fk{Y}\to\fk{Y}$ be the projections, so that the diagram
\[
  \xymatrix{
    \fk{X} \ar[d]_f
    & \fk{X}\times_\fk{G}\fk{Y} \ar[l]^p \ar[d]^q\\
    \fk{G}
    & \fk{Y} \ar[l]_g
  }
\]
...
\begin{equation*}
    u:g^*(f_*(\sh{F})) \xrightarrow{\sim} q_*(p^*(\sh{F})).\tag{10.14.6.1}
\end{equation*}
\end{cor}
...
To see that $u$ is an isomorphism, we can immediately restrict to the case where $\fk{G}$, $\fk{X}$, and $\fk{Y}$ are formal spectra of Noetherian adic rings $A$, $B$, and $C$ (respectively), with the conditions above in \sref{1.10.14.5}[ii];
