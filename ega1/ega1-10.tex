...
\]
is exact.
We have that $\sh{F}|U=\wt{M}$, $\sh{F}'|U=\wt{M'}$, $\sh{F}''|U=\wt{M''}$, where $M$, $M'$, $M''$ are three $A$-modules of finite type such that the sequence $0\to M'\to M\to M''\to 0$ is exact (\sref{1.1.5.1} and \sref{1.1.3.11}); let $\sh{J}\in\Phi$ are let $\fk{J}$ be an ideal of $A$ such that $\sh{J}|U=\wt{\fk{J}}$.
We then have
\[
