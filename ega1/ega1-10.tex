...
\label{1.10.9.10}
\oldpage[I]{201}
If $S$ is the sum $X_1\sqcup X_2$ \sref{subsection:1.3.1}, $X'$ the union $X_1'\cup X_2'$, where $X_i'$ is a closed subset of $X_i$ ($i=1,2$), then we have $X_{/X'}={X_1}_{/X_1'}\sqcup{X_2}_{/X_2'}$.
\end{rmk}
...
Finally, as $\Hom_A(M,N)$ is an $A$-module of finite type, we can apply (i), which identifies $\Gamma(\mathfrak{X},(\Hom_A(M,N))^\Delta)$ with $\Hom_A(M,N)$, and we use \sref{1.10.10.2.2}, which proves that the homomorphism \sref{1.10.10.2.3} is an isomorphism.
\end{proof}
