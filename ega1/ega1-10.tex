...
\begin{env}[10.12.1]
\label{1.10.12.1}
Let $\fk{X}$ and $\fk{G}$ be two \emph{locally Noetherian} formal preschemes;
we say that a morphism $f:\fk{X}\to\fk{G}$ is \emph{adic} if there exists an ideal of definition $\sh{J}$ of $\fk{G}$ such that $\sh{K}=f^*(\sh{J}\OO_\fk{X}$ is an ideal of definition of $\fk{X}$;
we then also say that $\fk{X}$ is an \emph{adic $\fk{G}$-prescheme} (for $f$).
Whenever this is the case, for \emph{every} ideal of definition $\sh{J}_1$ of $\fk{G}$, $\sh{K}_1=f^*(\sh{J}_1)\OO_\fk{X}$ is an ideal of definition of $\fk{X}$.
Indeed, the question being local, we can assume that $\fk{X}$ and $\fk{G}$ are Noetherian and affine;
...
It follows immediately from the above that, if $\fk{X}$ and $\fk{Y}$ are adic $\fk{G}$-preschemes, then \emph{every $\fk{G}$-morphism $u:\fk{X}\to\fk{Y}$ is adic}:
indeed, if $f:\fk{X}\to\fk{G}$ and $g:\fk{Y}\to\fk{G}$ are the structure morphisms, and $\sh{J}$ is an ideal of definition of $\fk{G}$, then we have $f=g\circ u$, and so $u^*(g^*(\sh{J})\OO_\fk{Y})\OO_\fk{X}=f^*(\sh{J})\OO_\fk{X}$ is an ideal of definition of $\fk{X}$, and, by hypothesis, $g^*(\sh{J})\OO_\fk{Y}$ is an ideal of definition of $\fk{Y}$.

\begin{env}[10.12.2]
\label{1.10.12.2}
In what follows, we suppose that we have some fixed locally Noetherian formal prescheme $\fk{G}$, and some ideal of definition $\sh{J}$ of $\fk{G}$;
we set $S_n=(\fk{G},\OO_\fk{G}/\sh{J}^{n+1})$.
The (locally Noetherian) adic $\fk{G}$-preschemes clearly form a \emph{category}.
...
\begin{thm}[10.12.3]
\label{1.10.12.3}
There is a canonical equivalence between the category of adic $\fk{G}$-preschemes and the category of adic inductive $(S_n)$-systems.
\end{thm}

The equivalence in question is obtained in the following way:
if $\fk{X}$ is an adic $\fk{G}$-prescheme, and $f:\fk{X}\to\fk{G}$ is the structure morphism, then $\sh{K}=f^*(\sh{J})\OO_\fk{X}$ is an ideal of definition of $\fk{X}$, and we associate to $\fk{X}$ the inductive system of the $X_n=(\fk{X},\OO_\fk{X}/\sh{K}^{n+1})$, with the stucure morphism $f_n:X_n\to S_n$ corresponding to $f$ \sref{1.10.5.6}.
...
Furthermore, it can be immediately verified that a $\fk{G}$-morphism $u:\fk{X}\to\fk{Y}$ of adic $\fk{G}$-preschemes corresponds (with the obvious notation)
\oldpage[I]{207}
...
let $\fk{X}$ be the inductive limit, and $f:\fk{X}\to\fk{G}$ the morphism given by the inductive limit of the inductive system $(f_n)$.
...
The question being local on $\fk{X}$ and $\fk{G}$, we can assume that $\fk{G}=\Spf(A)$, $\sh{J}=\fk{J}^\Delta$ (with $A$ a Noetherian $\fk{J}$-adic ring), and $X_n=\Spec(B_n)$;
...
\begin{env}[10.12.3.2]
\label{1.10.12.3.2}
The above equivalence gives, for adic $\fk{G}$-preschemes $\fk{X}$ and $\fk{Y}$, a \emph{canonical bijection}
\[
    \Hom_\fk{G}(\fk{X},\fk{Y}) \xrightarrow{\sim} \varprojlim_n\Hom_{S_n}(X_n,Y_n)
\]
...
Indeed, $\Delta_{\fk{X}|\fk{G}}$ is then the inductive limit of the sequence of morphisms $\Delta_{X_n|S_n}$ \sref{1.10.7.4}, and the image of the underlying space of $\fk{X}$ (resp. of $\fk{X}\times_\fk{G}\fk{X}$ under $\Delta_{\fk{X}|\fk{G}}$) is identical to the image of the underlying space of $X_0$ (resp. of $X_0\times_{S_0}X_0$) under $\Delta_{X_0|S_0}$;
...
With the notation of \sref{1.10.15.2}, we in fact have $(g\circ f)_0=g_0\circ f_0$, and $(f\times_\fk{G}g)_0=f_0\times_{S_0}g_0$;
...
\oldpage[I]{213}
(by replacing ``affine open subset'' by ``formal affine open subset satisfying condition \emph{(b)} of \sref{1.10.6.3}'').
...
then $\fk{X}\times_\fk{G}\fk{Y}$ is the inductive limit of the sequence $(X_n\times_{S_n}Y_n)$, and $\Gamma_f$ the inductive limit of the sequence $(\Gamma_{f_n})$ \sref{1.10.7.4};
...
If we show that $\Gamma_{f_U}$ is a closed immersion, then $\Gamma_f$ will be a closed immersion \sref{1.10.14.4}, or, in other words, we are led to consider the case where $\fk{G}=\Spf(A)$, $\fk{X}=\Spf(B)$, and $\fk{Y}=\Spf(C)$ are affine (with $A$, $B$, and $C$ Noetherian adics), with $f$ corresponding to a continuous $A$-homomorphism $\vphi:C\to B$;
...
Indeed, with the notation of \sref{1.10.8.5}, for $\wh{X}$ to be separated, it is necessary and sufficient for $X'_0$ to be separated \sref{1.10.15.2}, and since $\wh{X}_\red=(X'_0)_\red$, it is equivalent to ask for $\wh{X}_\red$ to be separated \sref{1.5.5.1}[vi].
\end{proof}
