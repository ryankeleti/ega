...
We then have for every integer $n>0$, $f^*(\sh{K}^n)\OO_\fk{X}=(f^*(\sh{K})\OO_\fk{X})^n\subset\sh{J}^n$; we can thus \sref{1.10.5.6} induce from $f$ a morphism of (usual) preschemes $f_n:X_n\to Y_n$, by setting $X_n=(\fk{X},\OO_\fk{X}/\sh{J}^{n+1})$ and $Y_n=(\sh{Y},\OO_\fk{Y}/\sh{K}^{n+1})$, and it immediately follows from the definitions that the diagrams
\[
  \xymatrix{
    X_m\ar[r]^{f_m}\ar[d] &
    Y_m\ar[d]\\
