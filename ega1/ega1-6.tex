...
So let $(\mathfrak{a}_n)$ be an increasing sequence of ideals of $A$; by a canonical bijective correspondence, there is a corresponding sequence $(\wt{\mathfrak{a}}_n)$ of sheaves of ideals in $\wt{A}=\OO_X$;
to see that the sequence $(\mathfrak{a}_n)$ is \unsure{stable}, it suffices to prove that the sequence $(\wt{\mathfrak{a}}_n)$ is.
But the restriction $\wt{\mathfrak{a}}_n|V_i$ is a quasi-coherent sheaf of ideals in $\OO_X|V_i$, being the inverse image of $\wt{\mathfrak{a}}_n$ under the canonical injection $V_i\to X$ \sref[0]{0.5.1.4};
...
to say that $x$ is contained in only one single irreducible component of $X$ is to say that $\mathfrak{j}_x$ contains only one single minimal ideal of $A$ \sref{1.1.1.14}, which is equivalent to saying that $\mathfrak{j}_x\OO_x$ contains only one single minimal ideal of $\OO_x$, whence the conclusion.
\end{proof}
