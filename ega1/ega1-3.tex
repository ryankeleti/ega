...
\begin{env}[3.2.9]
\label{1.3.2.9}
\footnote{\emph{[Trans.] \sref{1.3.2.9} is from the errata of EGA~II, on page 221, whence the change in page numbering.}}
\oldpage[II]{221}
...
homomorphisms $\sh{O}_x\to A$ for $x\in X$ \sref{1.2.2.4}; we say that the
...
\begin{proof}
\label{proof-1.3.6.1}
Since $\Spec(\sh{O}_y/\mathfrak{a}_y)\to Y$ is radicial (\sref{1.3.5.4} and \sref{1.2.4.7}), since $\Spec(\sh{O}_y/\mathfrak{a}_y)$ is a single point, and since the ideal $\mathfrak{m}_y/\mathfrak{a}_y$ is nilpotent by hypothesis \sref{1.1.1.12}, we already know (\sref{1.3.5.10} and \sref{1.3.3.4}) that $p$ identifies, as \emph{sets}, the underlying space of $X\times_Y\Spec(\sh{O}_y/\mathfrak{a}_y)$ with $f^{-1}(y)$; everything reduces to proving that $p$ is a homeomorphism.
