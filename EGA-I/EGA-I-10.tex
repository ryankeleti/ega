...
\begin{cor}[10.8.6]
\label{1.10.8.6}
The (usual) prescheme $\wh{X}_\mathrm{red}$ is the unique reduced subprescheme of $X$ having $X'$ for its underlying space \sref{1.5.2.1}.
...
We leave it to the reader to state, for the same type of formal preschemes and morphisms as in \sref{1.10.15.3}, the propositions corresponding to \sref{1.5.5.5}, \sref{1.5.5.9}, and \sref{1.5.5.10} (by replacing ``affine open subset'' by ``formal affine open subset satisfying condition \emph{(b)} of \sref{1.10.6.3}'').
...
Indeed, with the notation of \sref{1.10.8.5}, for $\wh{X}$ to be separated, it is necessary and sufficient for $X'_0$ to be separated \sref{1.10.15.2}, and since $\wh{X}_\mathrm{red}=(X'_0)_\mathrm{red}$, it is equivalent to ask for $\wh{X}_\mathrm{red}$ to be separated \sref{1.5.5.1}[vi].
\end{proof}
