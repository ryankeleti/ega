...
\begin{proof}
\label{proof-1.7.4.6}
By \sref{1.7.3.5} and \sref{1.7.4.1}, the equations $\sh{F}_x=0$ and $\sh{F}\otimes_{\OO_X}\sh{R}(X)=0$ are equivalent, so \emph{a)} and \emph{b)} are equivalent; also, $\Supp(\sh{F})$ is closed in $X$ \sref[0]{0.5.2.2}, and since every nonempty open subset of $X$ contains $x$, \emph{b)} and \emph{c)} are equivalent.
\end{proof}
