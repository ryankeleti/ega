...
For every open affine $U$ of $X$, every section of $\Omega_{X/S}^p$ over $U$ is (because of \rm{(ii)}) a linear combination if a finite number of elements of the form $g(df_1 \wedge df_2 \wedge \dots \wedge df_p)$, where $g$ and the $f_i$ are sections of $\sh{O}_X$ over $U$ \sref[0]{0.20.4.7}.
