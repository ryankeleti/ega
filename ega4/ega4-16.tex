...
\oldpage[IV-4]{35}
  ...
  \[
    \label{IV.16.6.2.1}
    d(\omega'_p \wedge \omega''_q) = (d\omega'_p) \wedge \omega''_q + (-1)^p \omega'_p \wedge \omega''_q.
    \tag{16.6.2.1}
  \]
\end{enumerate}
...
it all comes down to seeing that, having \sref{IV.16.6.2.2} in mind, that there is an $A$-homomorphism $u$ of $\Omega_{B/A}^1$ in $\Omega_{B/A}^2$ such that we have 
\[
  \label{IV.16.6.2.3}
  u(g.df) = dg \wedge df
  \tag{16.6.2.3}
...
\[
  \label{IV.16.6.4.1}
  \Omega_{X/S}^\bullet \otimes_S S' \to \Omega_{X'/S'}^1
  \tag{16.6.4.1}
\]
deduced from the isomorphism \sref{IV.16.5.9.1} is bijective.
...
\begin{proof}
The first claim is immediate, the formation of the exterior algebra of a module commutes with every extension of the scalar ring.
...
Then, for every integer $p \geq 1$, the $p$-differentials $ds_{i_1} \wedge ds_{i_2} \wedge \dots \wedge ds_{i_p}$ (for $i_1 \leq i_2 \leq \dots \leq i_p$ elements of $[1,n]$) form a basis of $\choose{n}{p}$ elements of $\Gamma(U, \Omega_{X/S}^p)$ over $\Gamma(U, \sh{O}_X)$.
