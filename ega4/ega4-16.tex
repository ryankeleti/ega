...
Because of \sref{IV.16.8.1}, given two open sets $U \supset V$ of $X$, we have a restriction homomorphism
\[
...
\[
  ...
  \tag{16.8.7.1}
\] 
where the horizontal arrows are the isomorphisms \sref{IV.16.8.4.2} and the horizontal arrow on the left comes from the canonical morphism $\sh{P}_{X/S}^m(\sh{F}) \to \sh{P}_{X/S}^n(\sh{F})$ \sref{IV.16.7.7}.
...
\begin{proof}
  ...
  \[
    \sum_{H' \subset I_{n+1}} (-1)^{|H'|} (\prod_{i \in H'} a_i) D_{a_{n+1}} ((\prod_{i \notin H'} a_i ) t ) = 0.
  \]
