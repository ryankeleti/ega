...
Then it exists one and only one morphism of $\sh{O}_S$-algebras
\[
  \label{IV.16.4.11.1}
...
Then $(\sh{P}_{X/S}^n)_x \otimes_{\sh{O}_x} \kres(x)$ is canonically isomorphic (as an augmented $\kres(x)$-algebra) to $\sh{O}_x/\mathfrak{m}^{n+1}$.
\end{corollary}
...
But then the isomorphism \sref{IV.16.4.15.2} is simply deduced from the canonical isomorphism $B \otimes_A B \to B \otimes_{R^{-1}} B$ by passing to the quotient \sref[I]{I.1.5.3}.
\end{proof}
...
\begin{proof}
This results from \sref{IV.16.4.8} and \sref{IV.16.4.14}, where we make $A = k$, $B = k[T_1, \dots, T_r]$ and $S = B \setmin \{ 0 \}$.
\end{proof}

One thus recover the fact that the $dT_i$ form a basis of the $K$-vector space $\Omega_{K/k}^1$ \sref[0]{20.5.10}.
