...
we can also say that we thus define on $\sh{P}_{X/S}^n$ \emph{two} structures of \emph{quasi-coherent augmented $\sh{O}_X$-algebras};
the corresponding structures of an $\sh{O}_X$-module on $\shGr_n(\sh{P}_{X/S}^n)$ are the same. 
We also have, by passing to the limit, two structures of $\sh{O}_X$-algebras on $\sh{P}_{X/S}^\infty$.	
\end{env}
...
This automorphism permutes the structure of $\sh{O}_X$-algebra on $\sh{P}_{X/S}^n$ and on $\sh{P}_{X/S}^\infty$.
\end{proposition}

\begin{env}[16.3.5]
\label{IV.16.3.5}
In what follows, the two structures of $\sh{O}_X$-algebras defined on the $\sh{P}_{X/S}^n$ and on $\sh{P}_{X/S}^\infty$ will play very different roles:
...
To prove the lemma we can, being a local problem, restrict to the case where $\sh{F} = \sh{F}' \oplus \sh{F}''$, $u$ and $v$ being the canonical homomorphisms.
...
we can easily see that it follows from the definition of $u$ \sref{IV.16.4.8.2} and of $\delta$ \sref{IV.16.4.7.1}, given the definition of the structure of $\sh{O}_X$-algebras of $\sh{P}_{X/S}^n$ and of the $d_{X/S}^n$ \sref{IV.16.3.5} and \sref{IV.16.4.3.6}.
...
\[
  ...
  \tag{16.4.11.1}
\]  
(via the structure of $\sh{O}_S$-algebra on $\sh{O}_{S_g^{(n)}}$ defined by $f$ \sref{IV.16.1.7}), making the diagram
\[
  \label{IV.16.4.11.2}
  \xymatrix{
...
Then the preceding definitions show that if $W$ is an open of $S$, $t$ is a section of $\sh{O}_X$ over $f^{-1}(W)$, $\varpi_n(d^nt|W)$ is equal to the canonical image of $t$ in $\Gamma(W, (\sh{O}_X/\sh{K}^{n+1})|W)$. 
