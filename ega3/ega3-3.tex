...
\begin{cor}[3.3.2]
\label{3.3.3.2}
Let $A$ be a Notherian ring, $\mathfrak{m}$ an ideal of $A$, $X$ a proper $A$-scheme, and $\sh{F}$ a coherent $\OO_X$-module.
...
\begin{proof}
\label{proof-3.3.3.2}
It suffices to apply Proposition~\sref{3.3.3.1} with $Y=\Spec(A)$, $\sh{S}=\wt{S}$, $\sh{M}_k=\mathfrak{m}^k\sh{F}$, taking into account Corollary~\sref{3.1.4.11}.
\end{proof}
...
\begin{enumerate}
  ...
  \item[2nd.] \emph{If $\sh{K}$ is a sheaf of ideals of definition for $\mathfrak{S}$ and if we set $\sh{J}=f^*(\sh{K})\OO_\mathfrak{X}$, $X_0=(\mathfrak{X},\OO_\mathfrak{X}/\sh{J})$, $S_0=(\mathfrak{S},\OO_\mathfrak{S}/\sh{K})$, then the morphism $f_0:X_0\to S_0$ induced by $f$~\sref[I]{1.10.5.6} is proper}.
\end{enumerate}
It is immediate that this definition does not depend on the sheaf of ideals of definition $\sh{K}$ for $\sh{S}$ considered; indeed, if $\sh{K}'$ is a second sheaf of ideals of definition such that $\sh{K}'\subset\sh{K}$, and if we set $\sh{J}'=f^*(\sh{K}')\OO_\mathfrak{X}$, $X_0'=(\mathfrak{X},\OO_\mathfrak{X}/\sh{J}')$, $S_0'=(\sh{S},\OO_\mathfrak{S}/\sh{K}')$, then the morphism $f_0':X_0'\to S_0'$ induced by $f$ is such that the diagram
\[
  \xymatrix{
...
We note that, for all $n\geq 0$, if we set $X_n=(\mathfrak{X},\OO_\mathfrak{X}/\sh{J}^{n+1})$, $S_n=(\mathfrak{S},\OO_\mathfrak{S}/\sh{K}^{n+1})$, then the morphism $f_n:X_n\to S_n$ induced by $f$~\sref[I]{1.10.5.6} is proper for all $n$ whenever it is for $n=0$~\sref[II]{2.5.4.6}.
...
For every coherent $\OO_\mathfrak{X}$-module $\sh{F}$, the $\OO_\mathfrak{Y}$-modules $\RR^q f_*(\sh{F})$ are coherent for all $q\geq 0$.
\end{thm}

Let $\sh{J}$ be a sheaf of ideals of definition for $\mathfrak{Y}$, $\sh{K}=f^*(\sh{J})\OO_\mathfrak{X}$, and consider the $\OO_\mathfrak{X}$-modules
\[
\label{3.3.4.2.1}
  \sh{F}_k=\sh{F}\otimes_{\OO_\mathfrak{Y}}(\OO_\mathfrak{Y}/\sh{J}^{k+1})=\sh{F}/\sh{K}^{k+1}\sh{F}\quad(k\geq 0)
  \tag{3.4.2.1}
\]
which evidently form a \emph{projective system} of topological $\OO_\mathfrak{X}$-modules, such that
\oldpage[III]{120}
$\sh{F}=\varprojlim_k\sh{F}_k$~\sref[I]{1.10.11.3}.
On the other hand, it follows from Theorem~\sref{3.3.4.2} that each of the $\RR^q f_*(\sh{F})$, being coherent, is naturally equipped with a topological $\OO_\mathfrak{Y}$-module structure~\sref[I]{1.10.11.6}, and so are the $\RR^q f_*(\sh{F}_k)$.
...
\]
which are necessarily continuous for the topological $\OO_\mathfrak{Y}$-module structures above~\sref[I]{1.10.11.6}, and form a projective system, giving the limit a canonical functorial homomorphism
\[
\label{3.3.4.2.2}
  \RR^q f_*(\sh{F})\to\varprojlim_k\RR^q f_*(\sh{F}_k),
  \tag{3.4.2.2}
\]
which will be a continuous homomorphism of topological $\OO_\mathfrak{Y}$-modules.
...
\]
We know that $\mathfrak{J}^\Delta$ is a sheaf of ideals of definition for $\mathfrak{Y}$~\sref[I]{1.10.3.1}; let $\sh{K}=f^*(\mathfrak{J}^\Delta)\OO_\mathfrak{X}$, $X_0=(\mathfrak{X},\OO_\mathfrak{X}/\sh{K})$, $Y_0=(\mathfrak{Y},\OO_\mathfrak{Y}/\mathfrak{J}^\Delta)=\Spec(A_0)$, with $A_0=A/\mathfrak{J}$.
It is clear that the $\sh{M}_k=\mathfrak{J}^k\sh{F}/\mathfrak{J}^{k+1}\sh{F}$ are coherent $\OO_{X_0}$-modules~\sref[I]{1.10.11.3}.
Consider on the other hand the quasi-coherent graded $\OO_{X_0}$-algebra
\[
\label{3.3.4.4.4}
  \sh{S}=\OO_{X_0}\otimes_{A_0}S=\gr(\OO_\mathfrak{X})=\bigoplus_{k\geq 0}\sh{K}^k/\sh{K}^{k+1}.
  \tag{3.4.4.4}
\]

The hypothesis that $\sh{F}$ is a $\OO_\mathfrak{X}$-module of finite type implies first that $\sh{M}$ is
\oldpage[III]{121}
a graded $\sh{S}$-module \emph{of finite type}.
Indeed, the question is local on $\mathfrak{X}$, and we can thus suppose that $\mathfrak{X}=\Spf(B)$, where $B$ is an adic Noetherian ring, and $\sh{F}=N^\Delta$, where $N$ is a $B$-module of finite type~\sref[I]{1.10.10.5}; we have in addition $X_0=\Spec(B_0)$, where $B_0=B/\mathfrak{J}B$, and the quasi-coherent $\OO_{X_0}$-modules $\sh{S}$ and $\sh{M}$ are respectively equal to $\wt{S'}$ and $\wt{M'}$, where $S'=\bigoplus_{k\geq 0}((\mathfrak{J}^k/\mathfrak{J}^{k+1})\otimes_{A_0}B_0)$ and $M'=\bigoplus_{k\geq 0}((\mathfrak{J}^k/\mathfrak{J}^{k+1})\otimes_{A_0}N_0)$, with $N_0=N/\mathfrak{J}N$; we then evidently have $M'=S'\otimes_{B_0}N_0$, and as $N_0$ is a $B_0$-module of finite type, $M'$ is a $S'$-module of finite type, hence our assertion~\sref[I]{1.1.3.13}.
...
Note that on the other hand, if we set $X_k=(\mathfrak{X},\OO_\mathfrak{X}/\sh{K}^{k+1})$, then $\sh{F}_k$ is a coherent $\OO_{X_k}$-module~\sref[I]{1.10.11.3}, and if $U$ is an affine open set in $X_0$, then $U$ is also an affine open set in each of the $X_k$~\sref[I]{1.5.1.9}, so $\HH^n(U,\sh{F}_k)=0$ for all $n>0$ and all $k$~\sref{3.1.3.1} and $\HH^0(U,\sh{F}_k)\to\HH^0(U,\sh{F}_h)$ is surjective for $h\leq k$~\sref[I]{1.1.3.9}.
...
Let us now prove that $\RR^n f_*(\sh{F})$ is a coherent $\OO_\mathfrak{Y}$-module, and more precisely that we have
\[
\label{3.3.4.5.1}
...
As~(3.4.2.2) is then an isomorphism of coherent $\OO_\mathfrak{Y}$-modules, it is necessarily a \emph{topological} isomorphism~\sref[I]{1.10.11.6}.
